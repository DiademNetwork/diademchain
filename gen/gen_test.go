...
func TestSpin(t *testing.T) {
	type spinTestParms struct {
		...
		// diadem spin <spinTestParms.spinUrl> --name <spinTestParms.name> --outdir /tmp/testspin123456789
		//
		// spinUrl describes the location of the data on the internet, either url in github format
		// or the name of a diadem project with hardcoded url parts; DiademUrlBase and DiademUrlEnd.
		//
	...
	for _, test := range spins {
		...
		// If referencing by diadem package name rather than URL, we amend the hard coded value
		...
		if strings.Contains(test.spinUrl, "/") {
			...
			DiademUrlBase = mockServer.URL + "/" + "somediadempath"
		}

		// Run command, the same as
		// diadem spin <spinArgument> --outdir <testDir> --name <test.name>
