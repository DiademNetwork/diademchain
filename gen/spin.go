...
var (
	...
	DiademUrlBase         = "https://github.com/diademnetwork"
	DiademUrlEnd          = "archive/master.zip"
)
...
func getRepoPath(spin string) (string, string, error) {
	...
	if l == 1 {
		return splitSpin[0], DiademUrlBase + "/" + splitSpin[0] + "/" + DiademUrlEnd, nil
	}
	...
	if format == ".zip" {
		...
		return "", "", fmt.Errorf("wrong spin format %q, diadem project or GitHub zipfile", spin)
	}
}
