...
func GetKarmaMiddleWare(
	...
	createKarmaContractCtx func(state diademchain.State) (contractpb.Context, error),
	...
	return diademchain.TxMiddlewareFunc(func(
		...
		if tx.Id == callId {
			...
			if tx.VmType == vm.VMType_EVM {
				isActive, err := karma.IsContractActive(ctx, diadem.UnmarshalAddressPB(msg.To))
				if err != nil {
					return res, errors.Wrapf(err, "determining if contract %v is active", diadem.UnmarshalAddressPB(msg.To).String())
				}
				if !isActive {
					return res, fmt.Errorf("contract %s is not active", diadem.UnmarshalAddressPB(msg.To).String())
				}
			}
		}
		...
		if oracleAddr != nil && origin.Compare(*oracleAddr) == 0 {
			...
			if !isCheckTx && r.Info == utils.DeployEvm {
				...
				if err := karma.AddOwnedContract(ctx, origin, diadem.UnmarshalAddressPB(dr.Contract)); err != nil {
		...
		if 1 == originKarma.Cmp(diadem.NewBigUIntFromInt(math.MaxInt64)) {
