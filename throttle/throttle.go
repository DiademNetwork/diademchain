...
type Throttle struct {
	...
	karmaContractAddress diadem.Address
...
func (t *Throttle) runThrottle(state diademchain.State, nonce uint64, origin diadem.Address, limit int64, txId uint32, key string) error {
...
func (t *Throttle) getKarmaForTransaction(karmaContractCtx contractpb.Context, origin diadem.Address, txId uint32) (*common.BigUInt, error) {
...
func (t *Throttle) getKarmaState(chainState diademchain.State) (diademchain.State, error) {
	contractState := chainState.WithPrefix(diadem.DataPrefix(t.karmaContractAddress))
