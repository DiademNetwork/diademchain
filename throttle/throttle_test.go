...
var (
	...
	contract = diadem.MustParseAddress("chain:0x9a1aC42a17AAD6Dbc6d21c162989d0f701074044")
	...
	sourceStates = []*ktypes.KarmaSource{
		{Name: "sms", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(2)}},
		{Name: "oauth", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1)}},
		{Name: "token", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1)}},
		{Name: karma.CoinDeployToken, Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(maxDeployCount)}},
	}

	userState = ktypes.KarmaState{ //types.BigUInt
		SourceStates:     sourceStates,
		DeployKarmaTotal: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1 * maxDeployCount)},
		CallKarmaTotal:   &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1*2 + 2*1 + 3*1)},
	}
)
...
func mockSignedTx(t *testing.T, sequence uint64, id uint32, vmType vm.VMType, to diadem.Address) auth.SignedTx {
