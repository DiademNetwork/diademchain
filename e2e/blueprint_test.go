...
func TestContractBlueprint(t *testing.T) {
	...
	for _, test := range tests {
		config, err := common.NewConfig(test.name, test.testFile, test.genFile, "", test.validators, test.accounts, 0)
		if err != nil {
			t.Fatal(err)
		}
		...
		cmd := exec.Cmd{
			Dir:  config.BaseDir,
			Path: binary,
			Args: []string{binary, "build", "-o", "blueprint-cli", "github.com/diademnetwork/go-diadem/cli/blueprint"},
		}
