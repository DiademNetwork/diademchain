...
func NewNode(ID int64, baseDir, diademPath, contractDir, genesisFile, yamlFile string) *Node {
	return &Node{
		ID:          ID,
		ContractDir: contractDir,
		DiademPath:    diademPath,
...
func (n *Node) Init(accounts []*Account) error {
	...
	// copy base diadem.yaml (if there is one) to the node directory so that the node takes it into
	// account when generating the default genesis
	if err := n.SetConfigFromYaml(accounts); err != nil {
		return errors.Wrapf(err, "reading diadem yaml file %s", n.BaseYaml)
	}
	diademYamlPath := path.Join(n.Dir, "diadem.yaml")
	if err := n.Config.WriteToFile(diademYamlPath); err != nil {
	...
	init := &exec.Cmd{
		Dir:  n.Dir,
		Path: n.DiademPath,
		Args: []string{n.DiademPath, "init", "-f"},
	}
	...
	// And then we looking for the autogen genesis from diadem to grap settings from it.
	...
	nodekey := &exec.Cmd{
		Dir:  n.Dir,
		Path: n.DiademPath,
		Args: []string{n.DiademPath, "nodekey"},
	}
	...
	fmt.Printf("running diadem init in directory: %s\n", n.Dir)
...
func (n *Node) Run(ctx context.Context, eventC chan *Event) error {
	...
	cmd := exec.CommandContext(ctx, n.DiademPath, "run", "--persistent-peers", n.PersistentPeers)
	cmd.Dir = n.Dir
	...
	for {
		...
		case event := <-eventC:
			...
			case ActionStop:
				...
				// restart
				time.Sleep(dur)
				cmd = exec.CommandContext(ctx, n.DiademPath, "run")
				cmd.Dir = n.Dir
				go func() {
		...
		case err := <-errC:
			...
			return err
		case <-ctx.Done():
			fmt.Printf("stopping diadem node %d\n", n.ID)
			return nil
		}
	}
}
