...
import (
	...
	d3types "github.com/diademnetwork/go-diadem/builtin/types/dposv3"
...
func CreateCluster(nodes []*Node, account []*Account) error {
	...
	for _, node := range nodes {
		...
		if err := node.SetConfigFromYaml(account); err != nil {
			return errors.Wrapf(err, "reading diadem yaml file %s", node.BaseYaml)
		}
		...
		node.Config.DiademCoinTransferGateway.DAppChainReadURI = fmt.Sprintf("http://127.0.0.1:%d/query", proxyAppPort)
		node.Config.DiademCoinTransferGateway.DAppChainWriteURI = fmt.Sprintf("http://127.0.0.1:%d/rpc", proxyAppPort)
		...
		diademYamlPath := path.Join(node.Dir, "diadem.yaml")
		if err := node.Config.WriteToFile(diademYamlPath); err != nil {
			return errors.Wrapf(err, "write config to %s", diademYamlPath)
		}
	...
	for _, node := range nodes {
		...
		if validator != nil {
			address := diadem.LocalAddressFromPublicKey(validator.PubKey)
	...
	for _, node := range nodes {
		...
		for _, contract := range gens.Contracts {
			...
			case "dposV3":
				...
				oracleAddr := diadem.LocalAddressFromPublicKey(validators[0].PubKey)
			...
			case "coin":
				...
				if len(init.Accounts) == 0 {
					for _, acct := range account {
						address, err := diadem.LocalAddressFromHexString(acct.Address)
						if err != nil {
							return err
						}
						addr := &types.Address{
			...
			case "chainconfig":
				...
				// set contract owner
				ownerAddr := diadem.LocalAddressFromPublicKey(validators[0].PubKey)
...
func modifyKarmaInit(contractInit json.RawMessage, accounts []*Account) (json.RawMessage, error) {
	...
	localOracle, err := diadem.LocalAddressFromHexString(accounts[0].Address)
	if err != nil {
