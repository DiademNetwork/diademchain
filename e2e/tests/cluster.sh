...
function init_cluster {
    ...
    $DIADEM_VALIDATORS_TOOL new \
        $FLAGS \
        --base-dir $CLUSTER_DIR \
        --contract-dir "" \
        --name cluster \
        --diadem-path $DIADEM_BIN \
        --force
...
function run_cluster {
    ...
    $DIADEM_VALIDATORS_TOOL run --conf cluster/runner.toml > cluster.log 2>&1 &
    DIADEM_PID=$!
    echo $DIADEM_PID > $CLUSTER_DIR/diadem.pid
}

function stop_cluster {
    if [[ -z "$DIADEM_PID" ]]; then
        DIADEM_PID=`cat ${CLUSTER_DIR}/diadem.pid`
        rm -f $CLUSTER_DIR/diadem.pid
    fi
    if [[ -n "$DIADEM_PID" ]]; then
        echo "Stopping cluster (${DIADEM_PID})"
        kill -15 "${DIADEM_PID}" &> /dev/null
    fi
    STOP_CLUSTER=false
}
