...
var (
	diademCmds = []string{"diadem", "example-cli", "blueprint-cli"}
)
...
func isDiademCmd(cmd string) bool {
	for _, diademCmd := range diademCmds {
		if path.Base(cmd) == diademCmd {
			return true
		}
	}
	return false
