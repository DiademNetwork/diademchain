...
import (
	...
	dposv2OracleCfg "github.com/diademnetwork/diademchain/builtin/plugins/dposv2/oracle/config"
	plasmacfg "github.com/diademnetwork/diademchain/builtin/plugins/plasma_cash/config"
	...
	receipts "github.com/diademnetwork/diademchain/receipts/handler"
	...
	"github.com/diademnetwork/diademchain/db"
...
type Config struct {
	...
	// When this setting is enabled Diadem EVM accounts are hooked up to the builtin ethcoin Go contract,
	// which makes it possible to use the payable/transfer features of the EVM to transfer ETH in
	// Solidity contracts running on the Diadem EVM. This setting is disabled by default, which means
...
// TODO: Move to diademchain/rpc package
//Structure for Diadem ENVINFO - ENV + Genesis + Diadem.yaml

type EnvInfo struct {
	Env         Env     `json:"env"`
	DiademGenesis Genesis `json:"diademGenesis"`
	DiademConfig  Config  `json:"diademConfig"`
}
...
func DefaultConfig() *Config {
	...
	cfg.DiademCoinTransferGateway = gateway.DefaultDiademCoinTGConfig(cfg.RPCProxyPort)
...
func (c *Config) AddressMapperContractEnabled() bool {
	return c.TransferGateway.ContractEnabled ||
		c.DiademCoinTransferGateway.ContractEnabled ||
...
func (c *Config) Clone() *Config {
	...
	clone.DiademCoinTransferGateway = c.DiademCoinTransferGateway.Clone()
...
func parseCfgTemplate() (*template.Template, error) {
	...
	var err error
	cfgTemplate, err = template.New("diademYamlTemplate").Parse(defaultDiademYamlTemplate)
	if err != nil {
		return nil, err
	}
	return cfgTemplate, nil
...
const defaultDiademYamlTemplate = `# Diadem Node config file
# See https://diademx.io/developers/docs/en/diadem-yaml.html for additional info.
# 
...
TransferGateway:
  ...
  {{end}}
  #
  # Diademcoin Transfer Gateway
  #
DiademCoinTransferGateway:
  # Enables the Transfer Gateway Go contract on the node, must be the same on all nodes.
  ContractEnabled: {{ .DiademCoinTransferGateway.ContractEnabled }}
  # Enables the in-process Transfer Gateway Oracle.
  # If this is enabled ContractEnabled must be set to true.
  OracleEnabled: {{ .DiademCoinTransferGateway.OracleEnabled }}
  # URI of Ethereum node the Oracle should connect to, and retrieve Mainnet events from.
  EthereumURI: "{{ .DiademCoinTransferGateway.EthereumURI }}"
  # Address of Transfer Gateway contract on Mainnet
  # e.g. 0x3599a0abda08069e8e66544a2860e628c5dc1190
  MainnetContractHexAddress: "{{ .DiademCoinTransferGateway.MainnetContractHexAddress }}"
  # Path to Ethereum private key on disk that should be used by the Oracle to sign withdrawals,
  # can be a relative, or absolute path
  MainnetPrivateKeyPath: "{{ .DiademCoinTransferGateway.MainnetPrivateKeyPath }}"
  # Path to DAppChain private key on disk that should be used by the Oracle to sign txs send to
  # the DAppChain Transfer Gateway contract
  DAppChainPrivateKeyPath: "{{ .DiademCoinTransferGateway.DAppChainPrivateKeyPath }}"
  DAppChainReadURI: "{{ .DiademCoinTransferGateway.DAppChainReadURI }}"
  DAppChainWriteURI: "{{ .DiademCoinTransferGateway.DAppChainWriteURI }}"
  # Websocket URI that should be used to subscribe to DAppChain events (only used for tests)
  DAppChainEventsURI: "{{ .DiademCoinTransferGateway.DAppChainEventsURI }}"
  DAppChainPollInterval: {{ .DiademCoinTransferGateway.DAppChainPollInterval }}
  MainnetPollInterval: {{ .DiademCoinTransferGateway.MainnetPollInterval }}
  # Oracle log verbosity (debug, info, error, etc.)
  OracleLogLevel: "{{ .DiademCoinTransferGateway.OracleLogLevel }}"
  OracleLogDestination: "{{ .DiademCoinTransferGateway.OracleLogDestination }}"
  # Number of seconds to wait before starting the Oracle.
  OracleStartupDelay: {{ .DiademCoinTransferGateway.OracleStartupDelay }}
  # Number of seconds to wait between reconnection attempts.
  OracleReconnectInterval: {{ .DiademCoinTransferGateway.OracleReconnectInterval }}
  # Address on from which the out-of-process Oracle should expose the status & metrics endpoints.
  OracleQueryAddress: "{{ .DiademCoinTransferGateway.OracleQueryAddress }}"
  {{if .DiademCoinTransferGateway.BatchSignFnConfig -}}
  BatchSignFnConfig:
    Enabled: {{ .DiademCoinTransferGateway.BatchSignFnConfig.Enabled }}
    LogLevel: "{{ .DiademCoinTransferGateway.BatchSignFnConfig.LogLevel }}"		
    LogDestination: "{{ .DiademCoinTransferGateway.BatchSignFnConfig.LogDestination }}"
    MainnetPrivateKeyPath: "{{ .DiademCoinTransferGateway.BatchSignFnConfig.MainnetPrivateKeyPath }}"
    MainnetPrivateKeyHsmEnabled: "{{ .DiademCoinTransferGateway.BatchSignFnConfig.MainnetPrivateKeyHsmEnabled }}"	
  {{end}}
