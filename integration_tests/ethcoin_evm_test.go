...
func TestEthCoinEvmIntegration(t *testing.T) {
	caller := diadem.MustParseAddress("chain:0xb16a379ec18d4093666f8f38b11a3071c920207d")
	contractAddr := diadem.RootAddress("chain")
...
func (c *ethCoinTestHelper) mint(
	ctx *plugin.FakeContextWithEVM, to diadem.Address, amount *big.Int,
) error {
	return ethcoin.Mint(c.contractCtx(ctx), to, diadem.NewBigUInt(amount))
}

func (c *ethCoinTestHelper) transfer(
	ctx *plugin.FakeContextWithEVM, from, to diadem.Address, amount *big.Int,
) error {
	return ethcoin.Transfer(c.contractCtx(ctx), from, to, diadem.NewBigUInt(amount))
}
...
func deployEthCoinIntegrationTestContract(
	ctx *plugin.FakeContextWithEVM, caller diadem.Address,
...
func (c *ethCoinIntegrationTestHelper) balance(ctx *plugin.FakeContextWithEVM, owner diadem.Address) (*big.Int, error) {
...
func (c *ethCoinIntegrationTestHelper) destroyContract(ctx *plugin.FakeContextWithEVM, receiver diadem.Address) error {
...
func (c *ethCoinIntegrationTestHelper) callEVM(ctx *plugin.FakeContextWithEVM, method string, params ...interface{}) error {
	...
	vm := evm.NewDiademVm(ctx.State, ctx.EvmDB, nil, nil, ctx.AccountBalanceManager, false)
	_, err = vm.Call(ctx.Message().Sender, c.Address, input, diadem.NewBigUIntFromInt(0))
	if err != nil {
		return err
	}
	return nil
...
func deployContractToEVM(ctx *plugin.FakeContextWithEVM, filename string, caller diadem.Address) (diadem.Address, error) {
	...
	vm := evm.NewDiademVm(ctx.State, ctx.EvmDB, nil, nil, nil, false)
