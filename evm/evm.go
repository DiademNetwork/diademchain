...
// EVMEnabled indicates whether or not Diadem EVM integration is available
...
func (m *evmAccountBalanceManager) AddBalance(account common.Address, amount *big.Int) error {
	...
	return m.abm.AddBalance(addr, diadem.NewBigUInt(amount))
}
...
func (m *evmAccountBalanceManager) SubBalance(account common.Address, amount *big.Int) error {
	...
	return m.abm.SubBalance(addr, diadem.NewBigUInt(amount))
}
...
func (m *evmAccountBalanceManager) SetBalance(account common.Address, amount *big.Int) error {
	...
	return m.abm.SetBalance(addr, diadem.NewBigUInt(amount))
}
...
func (m *evmAccountBalanceManager) Transfer(from, to common.Address, amount *big.Int) {
	...
	fromAddr := diadem.Address{
		ChainID: m.chainID,
		Local:   from.Bytes(),
	}
	toAddr := diadem.Address{
		ChainID: m.chainID,
		Local:   to.Bytes(),
	}
	...
	m.abm.Transfer(fromAddr, toAddr, diadem.NewBigUInt(amount))
}
...
func NewEvm(sdb vm.StateDB, lstate diademchain.State, abm *evmAccountBalanceManager, debug bool) *Evm {
	p := new(Evm)
	p.sdb = sdb
...
func (e Evm) Create(caller diadem.Address, code []byte, value *diadem.BigUInt) ([]byte, diadem.Address, error) {
	var err error
	var usedGas uint64
	...
	diademAddress := diadem.Address{
		ChainID: caller.ChainID,
		Local:   address.Bytes(),
	}
	return runCode, diademAddress, err
}

func (e Evm) Call(caller, addr diadem.Address, input []byte, value *diadem.BigUInt) ([]byte, error) {
	var err error
	var usedGas uint64
	defer func(begin time.Time) {
		lvs := []string{"method", "DeliverTx", "error", fmt.Sprint(err != nil)}
...
func (e Evm) StaticCall(caller, addr diadem.Address, input []byte) ([]byte, error) {
	origin := common.BytesToAddress(caller.Local)
	contract := common.BytesToAddress(addr.Local)
	vmenv := e.NewEnv(origin)
...
func (e Evm) GetCode(addr diadem.Address) []byte {
