...
func mockState() diademchain.State {
	header := abci.Header{}
...
func TestProcessDeployTx(t *testing.T) {
	...
	// Do not test EVM and diademEVM as they do not return a txReceipt
	// so they can be considered to not satisfy the intent of the VM interface.
	// They are also adequately tested by using the diademVM object
	...
	diademvm, err := manager.InitVM(lvm.VMType_EVM, mockState())
	require.Nil(t, err)
	testCryptoZombies(t, diademvm, caller)
	testDiademTokens(t, diademvm, caller)
...
// Test that we can access the diadem precompiles using solidity assembly block
...
func TestPrecompilesAssembly(t *testing.T) {
	...
	AddDiademPrecompiles()
	require.Equal(t, numEthPreCompiles+numDiademPreCompiles, len(ethvm.PrecompiledContractsByzantium))
...
func testMsgValue(t *testing.T, abiGP abi.ABI, caller, gPAddr diadem.Address, vm lvm.VM) {
	input, err := abiGP.Pack("msgValue")
	require.NoError(t, err, "packing parameters")
	_, err = vm.Call(caller, gPAddr, input, diadem.NewBigUIntFromInt(7))
...
func testNow(t *testing.T, abiGP abi.ABI, caller, gPAddr diadem.Address, vm lvm.VM) {
...
func testBlockTimeStamp(t *testing.T, abiGP abi.ABI, caller, gPAddr diadem.Address, vm lvm.VM) {
...
func testBlockNumber(t *testing.T, abiGP abi.ABI, caller, gPAddr diadem.Address, vm lvm.VM) {
...
func testTxOrigin(t *testing.T, abiGP abi.ABI, caller, gPAddr diadem.Address, vm lvm.VM) {
...
func testMsgSender(t *testing.T, abiGP abi.ABI, caller, gPAddr diadem.Address, vm lvm.VM) {
...
func deploySolContract(t *testing.T, caller diadem.Address, filename string, vm lvm.VM) (abi.ABI, diadem.Address) {
	...
	_, addr, err := vm.Create(caller, bytecode, diadem.NewBigUIntFromInt(0))
