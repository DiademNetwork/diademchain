...
type DiademEthDB struct {
	state      store.KVStore
	lock       sync.RWMutex
	logContext *ethdbLogContext
}

func NewDiademEthDB(_state diademchain.State, logContext *ethdbLogContext) *DiademEthDB {
	p := new(DiademEthDB)
...
func (s *DiademEthDB) Put(key []byte, value []byte) error {
	s.state.Set(key, value)
	return nil
}

func (s *DiademEthDB) Get(key []byte) ([]byte, error) {
	return s.state.Get(key), nil
}

func (s *DiademEthDB) Has(key []byte) (bool, error) {
	return s.state.Has(key), nil
}

func (s *DiademEthDB) Delete(key []byte) {
	s.state.Delete(key)
}

func (s *DiademEthDB) Close() {
}

func (s *DiademEthDB) NewBatch() ethdb.Batch {
	if LogEthDbBatch {
...
func (s *DiademEthDB) NewLogBatch(logContext *ethdbLogContext) ethdb.Batch {
	b := new(LogBatch)
	b.batch = *new(batch)
