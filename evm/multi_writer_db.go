...
func NewMultiWriterDB(evmDB dbm.DB, diademEthDB *DiademEthDB, logContext *MultiWriterDBLogContext, config MultiWriterDBConfig) MultiWriterDB {
...
func (db *multiWriterDB) Delete(key []byte) error {
	...
	if db.config.Write == DIADEM_ETH_DB || db.config.Write == ALL_DB {
		db.diademEthDB.Delete(key)
	}
	return nil
}
