...
type AddressMapperTestSuite struct {
	...
	validEthAddr    diadem.Address
	invalidEthAddr  diadem.Address
	validDAppAddr   diadem.Address
	invalidDAppAddr diadem.Address
}
...
func (s *AddressMapperTestSuite) SetupTest() {
	...
	r.NoError(err)
	ethLocalAddr, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(s.validEthKey.PublicKey).Hex())
	r.NoError(err)
	s.invalidEthAddr = diadem.Address{ChainID: "", Local: ethLocalAddr}
	s.validEthAddr = diadem.Address{ChainID: "eth", Local: ethLocalAddr}
	s.invalidDAppAddr = diadem.Address{ChainID: "", Local: addr1.Local}
	s.validDAppAddr = diadem.Address{ChainID: "chain", Local: addr1.Local}
...
func (s *AddressMapperTestSuite) TestAddressMapperHasIdentityMapping() {
	...
	ctx := contract.WrapPluginContext(
		plugin.CreateFakeContext(s.validDAppAddr /*caller*/, diadem.RootAddress("chain") /*contract*/),
	)
