...
var (
	...
	addr3     = diadem.MustParseAddress("chain:0x5cecd1f7261e1f4c684e297be3edf03b825e01c5")
	user_addr = addr3
	...
	sourceStates = []*ktypes.KarmaSource{
		{Name: "sms", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1)}},
		{Name: "oauth", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5)}},
		{Name: "token", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(10)}},
	}

	extremeSourceStates = []*ktypes.KarmaSource{
		{Name: "sms", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1000)}},
		{Name: "oauth", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5000)}},
		{Name: "token", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(10)}},
	}
...
func TestKarmaCoin(t *testing.T) {
	...
	require.NoError(t, coinContract.Approve(coinCtx, &coin.ApproveRequest{
		...
		Amount:  &types.BigUInt{Value: *diadem.NewBigUIntFromInt(200)},
	}))
	...
	err = karmaContract.DepositCoin(ctx, &ktypes.KarmaUserAmount{User: user, Amount: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(17)}})
	require.NoError(t, err)
	balAfterDeposit, err := coinContract.BalanceOf(coinCtx, &coin.BalanceOfRequest{Owner: user})
	require.NoError(t, err)
	expected := common.BigZero()
	expected = expected.Sub(&initalBal.Balance.Value, diadem.NewBigUIntFromInt(17))
	...
	err = karmaContract.WithdrawCoin(ctx, &ktypes.KarmaUserAmount{User: user, Amount: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5)}})
	require.NoError(t, err)
	balAfterWithdrawal, err := coinContract.BalanceOf(coinCtx, &coin.BalanceOfRequest{Owner: user})
	require.NoError(t, err)
	expected = expected.Sub(&initalBal.Balance.Value, diadem.NewBigUIntFromInt(17-5))
	...
	err = karmaContract.WithdrawCoin(ctx, &ktypes.KarmaUserAmount{User: user, Amount: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(500)}})
	require.Error(t, err)
}
...
func TestKarmaLifeCycleTest(t *testing.T) {
	...
	state, err = contract.GetUserState(ctx, ko)
	require.NoError(t, err)
	require.Equal(t, []*ktypes.KarmaSource{{Name: "token", Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(10)}}}, state.SourceStates)
