...
// TODO: should take diadem.Address instead
...
func (k *Karma) Init(ctx contract.Context, req *ktypes.KarmaInitRequest) error {
	...
	for _, userKarma := range req.Users {
		userAddr := diadem.UnmarshalAddressPB(userKarma.User)
...
func (k *Karma) WithdrawCoin(ctx contract.Context, req *ktypes.KarmaUserAmount) error {
	...
	amount := req.Amount.Value.Mul(&req.Amount.Value, diadem.NewBigUIntFromInt(-1))
...
func GetOracleAddress(ctx contract.StaticContext) (*diadem.Address, error) {
	...
	oracleAddr := diadem.UnmarshalAddressPB(&oraclePB)
...
func (k *Karma) GetUserState(ctx contract.StaticContext, user *types.Address) (*ktypes.KarmaState, error) {
	return GetUserState(ctx, diadem.UnmarshalAddressPB(user))
}

func GetUserState(ctx contract.StaticContext, userAddr diadem.Address) (*ktypes.KarmaState, error) {
...
func (k *Karma) AddKarma(ctx contract.Context, req *ktypes.AddKarmaRequest) error {
	...
	return AddKarma(ctx, diadem.UnmarshalAddressPB(req.User), req.KarmaSources)
}

// TODO: probably should rename ktypes.KarmaSource -> ktypes.KarmaAmount
func AddKarma(ctx contract.Context, userAddr diadem.Address, karmaAmounts []*ktypes.KarmaSource) error {
...
func (k *Karma) GetUserKarma(ctx contract.StaticContext, userTarget *ktypes.KarmaUserTarget) (*ktypes.KarmaTotal, error) {
	total, err := GetUserKarma(ctx, diadem.UnmarshalAddressPB(userTarget.User), userTarget.Target)
	if err != nil {
		return nil, err
	}
...
func GetUserKarma(ctx contract.StaticContext, userAddr diadem.Address, target ktypes.KarmaSourceTarget) (*common.BigUInt, error) {
...
func (c *Karma) registerOracle(ctx contract.Context, pbOracle *types.Address, currentOracle *diadem.Address) error {
	if pbOracle == nil {
		return fmt.Errorf("oracle address cannot be null")
	}

	newOracleAddr := diadem.UnmarshalAddressPB(pbOracle)
	...
	for _, c := range karmaSources.Sources {
		for _, s := range karmaStates.SourceStates {
			if c.Name == s.Name && (c.Target == ktypes.KarmaSourceTarget_DEPLOY) {
				reward := diadem.NewBigUIntFromInt(c.Reward)
