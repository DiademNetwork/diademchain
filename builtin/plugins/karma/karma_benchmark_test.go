...
import (
	...
	"github.com/diademnetwork/go-diadem/builtin/types/karma"
...
type testFunc func(state diademchain.State)
...
func calculateKarma(state diademchain.State) {
	const user = 0
...
func readKarma(state diademchain.State) {
	var err error
	const user = 0
...
func updateKarma(state diademchain.State) {
...
func mockUsers(state diademchain.State, sources karma.KarmaSources, logUsers int) diademchain.State {
	...
	for _, source := range sources.Sources {
		userState.SourceStates = append(userState.SourceStates, &karma.KarmaSource{
			Name:  source.Name,
			Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5)},
		})
	}
...
func mockState(logSize int) diademchain.State {
	header := abci.Header{}
	state := diademchain.NewStoreState(context.Background(), store.NewMemStore(), header, nil, nil)
...
func mockSources(state diademchain.State, logSize int) (diademchain.State, karma.KarmaSources) {
