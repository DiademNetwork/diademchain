...
func ContractActiveKey(owner diadem.Address, contractId uint64) ([]byte, error) {
...
func (k *Karma) SetActive(ctx contract.Context, contract *types.Address) error {
	record, err := GetContractRecord(ctx, diadem.UnmarshalAddressPB(contract))
	if err != nil {
		return err
	}
	if record.Owner == nil {
	...
	activeContractKey, err := ContractActiveKey(diadem.UnmarshalAddressPB(record.Owner), record.ContractId)
	if err != nil {
		return errors.Wrapf(err, "making contract id key from %v", record.ContractId)
	}
	if ctx.Has(activeContractKey) {
		return errors.Wrapf(err, "contract %v already active", diadem.UnmarshalAddressPB(record.Address).String())
	}
	...
	ownerAddr := diadem.UnmarshalAddressPB(record.Owner)
	if err := incrementOwnedContracts(ctx, diadem.UnmarshalAddressPB(record.Owner), 1); err != nil {
...
func DeactivateContract(ctx contract.Context, record *ktypes.KarmaContractRecord) error {
	...
	if !ctx.Has(activeContractKey) {
		return errors.Wrapf(err, "contract %v is not active", diadem.UnmarshalAddressPB(record.Address).String())
	}
...
func GetContractRecord(ctx contract.StaticContext, contractAddr diadem.Address) (*ktypes.KarmaContractRecord, error) {
...
func IsContractActive(ctx contract.StaticContext, contract diadem.Address) (bool, error) {
	var record ktypes.KarmaContractRecord
...
func AddOwnedContract(ctx contract.Context, owner, contract diadem.Address) error {
...
func GetActiveContractRecords(ctx contract.StaticContext, owner diadem.Address) ([]*ktypes.KarmaContractRecord, error) {
	...
	for _, kv := range activeRecords {
		...
		record, err := GetContractRecord(ctx, diadem.UnmarshalAddressPB(&contractAddr))
		if err != nil {
			return nil, err
		}
...
func GetActiveUsers(ctx contract.StaticContext) (map[string]ktypes.KarmaState, error) {
	...
	for _, kv := range activeUsers {
		...
		if userState.NumOwnedContracts > 0 {
			...
			owner := diadem.UnmarshalAddressPB(&ownerPB)
...
func incrementOwnedContracts(ctx contract.Context, owner diadem.Address, amount int64) error {
...
func deployUpkeep(ctx contract.Context, params ktypes.KarmaUpkeepParams, activeUsers map[string]ktypes.KarmaState, karmaSources []*ktypes.KarmaSourceReward) {
	...
	for userStr, userState := range activeUsers {
		user, err := diadem.ParseAddress(userStr)
		if err != nil {
			log.Error("cannot parse user %v during karma upkeep. %v", userStr, err)
			continue
		}

		upkeepCost := diadem.NewBigUIntFromInt(userState.NumOwnedContracts * params.Cost)
		paramCost := diadem.NewBigUIntFromInt(params.Cost)
		...
		if userKarma.Cmp(upkeepCost) >= 0 {
			...
			payKarma(canAfford.Mul(canAfford, diadem.NewBigUIntFromInt(params.Cost)), &userState, karmaSources, sourceMap)
			if leftOver == nil || leftOver.Cmp(big.NewInt(0)) == 0 {
				userState.DeployKarmaTotal = diadem.BigZeroPB()
			} else {
...
func setInactiveContractIdOrdered(ctx contract.Context, user diadem.Address, numberToInactivate uint64) error {
