...
type mockDiademCoinGateway struct {
}

func (m *mockDiademCoinGateway) Meta() (plugin.Meta, error) {
	return plugin.Meta{
...
func (m *mockDiademCoinGateway) DummyMethod(ctx contractpb.Context, req *MintToGatewayRequest) error {
	return nil
}
...
func TestMintToGateway(t *testing.T) {
	...
	mockDiademCoinGatewayContract := contractpb.MakePluginContract(&mockDiademCoinGateway{})

	pctx := plugin.CreateFakeContext(addr1, addr1)

	diademcoinTGAddress := pctx.CreateContract(mockDiademCoinGatewayContract)
	pctx.RegisterContract("diademcoin-gateway", diademcoinTGAddress, diademcoinTGAddress)
	...
	contract.Init(ctx, &InitRequest{
		Accounts: []*InitialAccount{
			&InitialAccount{
				Owner:   diademcoinTGAddress.MarshalPB(),
				Balance: uint64(29),
			},
			&InitialAccount{
				Owner:   addr1.MarshalPB(),
	...
	diademcoinTGBalance := big.NewInt(0).Mul(multiplier, big.NewInt(29))
	addr1Balance := big.NewInt(0).Mul(multiplier, big.NewInt(31))
	totalSupply := big.NewInt(0).Add(diademcoinTGBalance, addr1Balance)
	...
	require.Nil(t, err)
	require.Equal(t, diademcoinTGBalance, gatewayBalnanceResponse.Balance.Value.Int)

	require.Nil(t, contract.MintToGateway(
		contractpb.WrapPluginContext(pctx.WithSender(diademcoinTGAddress)),
		&MintToGatewayRequest{
			Amount: &types.BigUInt{
				Value: *diadem.NewBigUIntFromInt(59),
			},
		},
	))

	newTotalSupply := big.NewInt(0).Add(totalSupply, big.NewInt(59))
	newDiademCoinTGBalance := big.NewInt(0).Add(diademcoinTGBalance, big.NewInt(59))
	...
	require.Nil(t, err)
	require.Equal(t, newDiademCoinTGBalance, gatewayBalnanceResponse.Balance.Value.Int)
}
...
func TestBurn(t *testing.T) {
	...
	require.Nil(t, contract.Burn(
		...
		&BurnRequest{
			Owner: addr2.MarshalPB(),
			Amount: &types.BigUInt{
				Value: *diadem.NewBigUIntFromInt(2),
			},
		},
	))
...
func TestBurnAccess(t *testing.T) {
	...
	require.EqualError(t, contract.Burn(ctx, &BurnRequest{
		Owner: addr1.MarshalPB(),
		Amount: &types.BigUInt{
			Value: *diadem.NewBigUIntFromInt(10),
		},
	}), "not authorized to burn Diadem coin", "only diademcoin gateway can call Burn")
	...
	), "diademcoin gateway should be allowed to call Burn")
...
func TestMintToGatewayAccess(t *testing.T) {
	...
	}), "not authorized to mint Diadem coin", "only diademcoin gateway can call MintToGateway")
	...
	), "diademcoin gateway should be allowed to call MintToGateway")
