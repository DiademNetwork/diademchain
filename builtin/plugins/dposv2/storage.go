...
func (sl ValidatorStatisticList) Get(address diadem.Address) *ValidatorStatistic {
	for _, stat := range sl {
...
func (sl ValidatorStatisticList) GetV2(address []byte) *ValidatorStatistic {
	for _, stat := range sl {
		statAddress := diadem.LocalAddressFromPublicKeyV2(stat.PubKey)
...
func (dl *DistributionList) IncreaseDistribution(delegator types.Address, increase diadem.BigUInt) error {
	distribution := dl.Get(delegator)
	if distribution == nil {
		*dl = append(*dl, &Distribution{Address: &delegator, Amount: &types.BigUInt{Value: increase}})
	} else {
		updatedAmount := diadem.BigUInt{big.NewInt(0)}
...
func (dl *DistributionList) ResetTotal(delegator types.Address) error {
	...
	if distribution == nil {
		...
		distribution.Amount = &types.BigUInt{Value: diadem.BigUInt{big.NewInt(0)}}
	}
	return nil
}
