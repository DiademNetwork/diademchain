...
import (
	...
	"github.com/diademnetwork/go-diadem/client/timelock"
	...
	dpos "github.com/diademnetwork/diademchain/builtin/plugins/dposv2"
...
func (t *timeLockWorker) FetchRequestBatch(identity *client.Identity, tally *d2types.RequestBatchTallyV2, latestBlock uint64) ([]*d2types.BatchRequestV2, error) {
	...
	for i, event := range tokenTimeLockCreationEvents {
		...
		candidateLocalAddress := diadem.LocalAddressFromPublicKey(candidatePubKey)

		lockDuration := event.Duration.Uint64()
		lockTime := event.ReleaseTime.Uint64()
		amount := *diadem.NewBigUInt(event.Amount)
		var bonus diadem.BigUInt
		if lockDuration == dpos.TierLocktimeMap[dpos.TierMap[0]] {
			...
			bonus = *diadem.NewBigUIntFromInt(0)
		}
...
func (o *Oracle) Init() error {
	o.identity = &client.Identity{
		...
		DiademSigner:     o.cfg.DAppChainClientCfg.Signer,
		DiademAddr: diadem.Address{
			ChainID: o.cfg.DAppChainClientCfg.ChainID,
			Local:   diadem.LocalAddressFromPublicKey(o.cfg.DAppChainClientCfg.Signer.PublicKey()),
		},
	}
	...
	o.status.OracleAddress = o.identity.DiademAddr.String()
