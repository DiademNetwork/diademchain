...
import (
	...
	d2types "github.com/diademnetwork/go-diadem/builtin/types/dposv2"
)
...
func TestRegisterWhitelistedCandidate(t *testing.T) {
	...
	whitelistAmount := diadem.BigUInt{big.NewInt(1000000000000)}
	...
	err = dposContract.ProcessRequestBatch(contractpb.WrapPluginContext(pctx.WithSender(oracleAddr)), &RequestBatch{
		Batch: []*d2types.BatchRequestV2{
			&d2types.BatchRequestV2{
				Payload: &d2types.BatchRequestV2_WhitelistCandidate{&WhitelistCandidateRequest{
					CandidateAddress: addr.MarshalPB(),
					Amount:           &types.BigUInt{Value: diadem.BigUInt{big.NewInt(1000000000000)}},
					LockTime:         10,
				}},
				Meta: &d2types.BatchRequestMetaV2{
					BlockNumber: 1,
...
func TestLockTimes(t *testing.T) {
	...
	approvalAmount := &types.BigUInt{Value: diadem.BigUInt{big.NewInt(300)}}
	delegationAmount := &types.BigUInt{Value: diadem.BigUInt{big.NewInt(100)}}
	...
	assert.Equal(t, delegation1Response.Delegation.Amount.Value.Cmp(diadem.NewBigUIntFromInt(0)), 0)
	...
	expectedDelegation.Mul(&delegationAmount.Value, diadem.NewBigUIntFromInt(1))
	...
	expectedDelegation.Mul(&delegationAmount.Value, diadem.NewBigUIntFromInt(2))
...
func TestDelegate(t *testing.T) {
	...
	err = dposContract.Unbond(contractpb.WrapPluginContext(dposCtx.WithSender(addr1)), &UnbondRequest{
		ValidatorAddress: addr1.MarshalPB(),
		Amount:           &types.BigUInt{Value: diadem.BigUInt{big.NewInt(1)}},
	})
...
func TestRedelegate(t *testing.T) {
	...
	delegationAmount := diadem.NewBigUIntFromInt(10000000)
	smallDelegationAmount := diadem.NewBigUIntFromInt(1000000)
	...
	err = dposContract.Redelegate(contractpb.WrapPluginContext(dposCtx.WithSender(delegatorAddress2)), &RedelegateRequest{
		FormerValidatorAddress: addr1.MarshalPB(),
		ValidatorAddress:       addr2.MarshalPB(),
		Amount:                 &types.BigUInt{Value: *diadem.NewBigUIntFromInt(-1000)},
	})
	require.NotNil(t, err)
	...
	err = dposContract.Redelegate(contractpb.WrapPluginContext(dposCtx.WithSender(delegatorAddress2)), &RedelegateRequest{
		FormerValidatorAddress: addr1.MarshalPB(),
		ValidatorAddress:       addr2.MarshalPB(),
		Amount:                 &types.BigUInt{Value: *diadem.NewBigUIntFromInt(100000000)},
	})
	require.NotNil(t, err)
...
func TestReward(t *testing.T) {
	...
	statistic := ValidatorStatistic{
		DistributionTotal: &types.BigUInt{Value: diadem.BigUInt{big.NewInt(0)}},
	...
	assert.Equal(t, statistic.DistributionTotal.Value.Cmp(&diadem.BigUInt{big.NewInt(490000000000)}), 1)
	assert.Equal(t, statistic.DistributionTotal.Value.Cmp(&diadem.BigUInt{big.NewInt(510000000000)}), -1)
}
...
func TestElectWhitelists(t *testing.T) {
	...
	addr5 := diadem.Address{
		ChainID: chainID,
		Local:   diadem.LocalAddressFromPublicKey(pubKey5),
	}
	...
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(490000000)}), 1)
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(510000000)}), -1)
	...
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(740000000)}), 1)
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(760000000)}), -1)
	...
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(990000000)}), 1)
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(1000000000)}), -1)
	...
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(1990000000)}), 1)
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(2000000000)}), -1)
...
func TestElect(t *testing.T) {
	...
	for i := 0; i < 10; i = i + 1 {
		...
		require.Nil(t, err)
		assert.Equal(t, claimResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(0)}), 1)
	}
	...
	newWhitelistAmount := diadem.BigUInt{big.NewInt(2000000000000)}
...
func TestValidatorRewards(t *testing.T) {
	...
	checkResponse, err := dposContract.CheckDistribution(contractpb.WrapPluginContext(dposCtx.WithSender(addr1)), &CheckDistributionRequest{Address: addr1.MarshalPB()})
	require.Nil(t, err)
	assert.Equal(t, checkResponse.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(0)}), 1)
	...
	halvedDelegator2Claim := diadem.NewBigUIntFromInt(0)
	halvedDelegator2Claim.Div(&delegator2Claim.Amount.Value, diadem.NewBigUIntFromInt(2))
...
func TestRewardTiers(t *testing.T) {
	...
	require.Nil(t, err)
	assert.Equal(t, delegator4Claim.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(0)}), 1)
	...
	require.Nil(t, err)
	assert.Equal(t, delegator5Claim.Amount.Value.Cmp(&diadem.BigUInt{big.NewInt(0)}), 1)

	maximumDifference := scientificNotation(1, tokenDecimals)
	difference := diadem.NewBigUIntFromInt(0)

	// Checking that Delegator2's claim is almost exactly twice Delegator1's claim
	scaledDelegator1Claim := CalculateFraction(*diadem.NewBigUIntFromInt(20000), delegator1Claim.Amount.Value, true)
	...
	scaledDelegator1Claim = CalculateFraction(*diadem.NewBigUIntFromInt(40000), delegator1Claim.Amount.Value, true)
	...
	scaledDelegator1Claim = CalculateFraction(*diadem.NewBigUIntFromInt(15000), delegator1Claim.Amount.Value, true)
	...
	expectedWeightedAmount := CalculateFraction(*diadem.NewBigUIntFromInt(40000), *smallDelegationAmount, true)
...
func TestRewardCap(t *testing.T) {
	...
	scaledDelegator3Claim := CalculateFraction(*diadem.NewBigUIntFromInt(15000), delegator3Claim.Amount.Value, true)
	...
	maximumDifference := diadem.NewBigUIntFromInt(3)
	assert.Equal(t, difference.Int.CmpAbs(maximumDifference.Int), -1)
}
...
func TestDedoublingDelegation(t *testing.T) {
	originalAmount := diadem.NewBigUIntFromInt(100)
	...
	expectedAmount.Div(originalAmount, diadem.NewBigUIntFromInt(2))
...
func TestPlasmaDelegationMigration(t *testing.T) {
	...
	someDelegator := diadem.MustParseAddress("default:0xDc93E46f6d22D47De9D7E6d26ce8c3b7A13d89Cb")
	someAmount := types.BigUInt{Value: *diadem.NewBigUIntFromInt(100)}
	...
	someValidator := diadem.MustParseAddress("default:0xDc93E46f6d22D47De9D7E6d26ce8c312314589Cb")
