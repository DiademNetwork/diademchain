...
var (
	...
	addr1 = diadem.MustParseAddress("default:0x27690aE5F91C620B13266dA9044b8c0F35CeFbdC")
	pub2  = []byte("5wYR5atUGpnpZ+oerOZ8hi3B4dddSlxe6Hd30ZuuYWgps")
	addr2 = diadem.MustParseAddress("default:0x7278ec96B05B44c643c07005577e9F060dAef4EF")
	pub3  = []byte("oTFzT+lt+ztuUQd9yuQbPAdZPmezuoOtOFCUULSqgmU=")
	addr3 = diadem.MustParseAddress("default:0x8364d3A808D586b908b6B18Fc726ff134408fbfE")
	pub4  = []byte("emvRy1THBgGbNw/j1m5hqpXaVIZLHVz/GHQ58mxyc3A=")
	addr4 = diadem.MustParseAddress("default:0x9c285B0CE29E29C557a06Ca3a27cf1F550a96f38")
)
...
func TestGetSetCandidateList(t *testing.T) {
	...
	// get
	cand1 := cands.Get(diadem.Address{ChainID: chainID, Local: addr1.Local})
	assert.NotNil(t, cand1)
	assert.Equal(t, cand1.PubKey, pub1)
	assert.Equal(t, 0, cand1.Address.Local.Compare(addr1.Local))

	cand4 := cands.Get(diadem.Address{ChainID: chainID, Local: addr4.Local})
	assert.Nil(t, cand4)
...
func TestCalcFraction(t *testing.T) {
	...
	assert.Equal(t, CalculateFraction(TierBonusMap[TierMap[0]], amount, true), amount)

	newAmount := *diadem.NewBigUIntFromInt(187500000)
	assert.Equal(t, CalculateFraction(TierBonusMap[TierMap[1]], amount, true), newAmount)

	newAmount = *diadem.NewBigUIntFromInt(250000000)
	assert.Equal(t, CalculateFraction(TierBonusMap[TierMap[2]], amount, true), newAmount)

	newAmount = *diadem.NewBigUIntFromInt(500000000)
	assert.Equal(t, CalculateFraction(TierBonusMap[TierMap[3]], amount, true), newAmount)
...
func TestCandidateDelete(t *testing.T) {
	...
	cand2 := cands.Get(diadem.Address{ChainID: chainID, Local: addr2.Local})
