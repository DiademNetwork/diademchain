...
import (
	...
	dtypes "github.com/diademnetwork/go-diadem/builtin/types/dposv2"
...
var (
	secondsInYear = diadem.BigUInt{big.NewInt(yearSeconds)}

	blockRewardPercentage     = diadem.BigUInt{big.NewInt(500)}
	doubleSignSlashPercentage = diadem.BigUInt{big.NewInt(500)}
	inactivitySlashPercentage = diadem.BigUInt{big.NewInt(100)}
	limboValidatorAddress     = diadem.MustParseAddress("limbo:0x0000000000000000000000000000000000000000")
...
func (c *DPOS) Init(ctx contract.Context, req *InitRequest) error {
	if ctx.FeatureEnabled(diademchain.DPOSVersion3Feature, false) {
	...
	state := &State{
		...
		// genesis
		LastElectionTime:          0,
		TotalValidatorDelegations: diadem.BigZeroPB(),
		TotalRewardDistribution:   diadem.BigZeroPB(),
	}
...
func (c *DPOS) Delegate(ctx contract.Context, req *DelegateRequest) error {
	...
	v2_1 := ctx.FeatureEnabled(diademchain.DPOSVersion2_1, false)

	candidates, err := loadCandidateList(ctx)
	if err != nil {
		return err
	}
	cand := candidates.Get(diadem.UnmarshalAddressPB(req.ValidatorAddress))
...
func (c *DPOS) Redelegate(ctx contract.Context, req *RedelegateRequest) error {
	...
	if req.ValidatorAddress.Local.Compare(limboValidatorAddress.Local) != 0 {
		...
		candidate := candidates.Get(diadem.UnmarshalAddressPB(req.ValidatorAddress))
		// Delegations can only be made to existing candidates
		if candidate == nil {
			return logDposError(ctx, errCandidateNotFound, req.String())
		}
...
func (c *DPOS) addCandidateToStatisticList(ctx contract.Context, req *WhitelistCandidateRequest) error {
	...
	statistic := statistics.Get(diadem.UnmarshalAddressPB(req.CandidateAddress))
	if statistic == nil {
		...
		statistics = append(statistics, &ValidatorStatistic{
			...
			DistributionTotal: diadem.BigZeroPB(),
			DelegationTotal:   diadem.BigZeroPB(),
			SlashPercentage:   diadem.BigZeroPB(),
		})
	} else {
...
func (c *DPOS) RemoveWhitelistedCandidate(ctx contract.Context, req *RemoveWhitelistedCandidateRequest) error {
	...
	if statistic == nil {
		...
		statistic.WhitelistAmount = diadem.BigZeroPB()
	}
...
func (c *DPOS) RegisterCandidate2(ctx contract.Context, req *RegisterCandidateRequest) error {
	...
	checkAddr := diadem.LocalAddressFromPublicKey(req.PubKey)
	...
	if (statistic == nil || common.IsZero(statistic.WhitelistAmount.Value)) && common.IsPositive(state.Params.RegistrationRequirement.Value) {
		// A currently unregistered candidate must make a diadem token deposit
...
func Elect(ctx contract.Context) error {
	...
	for validator := range newDelegationTotals {
		delegationResults = append(delegationResults, &DelegationResult{
			ValidatorAddress: diadem.MustParseAddress(validator),
	...
	for _, res := range delegationResults[:validatorCount] {
		...
		if candidate != nil {
			...
			statistic := statistics.Get(diadem.UnmarshalAddressPB(candidate.Address))
			if statistic == nil {
				statistics = append(statistics, &ValidatorStatistic{
					...
					SlashPercentage:   diadem.BigZeroPB(),
					WhitelistAmount:   diadem.BigZeroPB(),
					WhitelistLocktime: 0,
				})
			} else {
...
func (c *DPOS) ListValidators(ctx contract.StaticContext, req *ListValidatorsRequest) (*ListValidatorsResponse, error) {
	...
	for _, validator := range validators {
		address := diadem.Address{ChainID: chainID, Local: diadem.LocalAddressFromPublicKey(validator.PubKey)}
...
func (c *DPOS) ListValidatorsSimple(
	...
	for _, validator := range validators {
		address := diadem.Address{
			ChainID: ctx.Block().ChainID,
			Local:   diadem.LocalAddressFromPublicKey(validator.PubKey),
		}
		stat := &ValidatorStatistic{
			PubKey:  validator.PubKey,
			Address: address.MarshalPB(),
...
func slash(ctx contract.Context, validatorAddr []byte, slashPercentage diadem.BigUInt) error {
	statistics, err := loadValidatorStatisticList(ctx)
	if err != nil {
		return err
	}
...
func loadCoin(ctx contract.Context, params *Params) *ERC20 {
	coinAddr := diadem.UnmarshalAddressPB(params.CoinContractAddress)
	return &ERC20{
		Context:         ctx,
...
func rewardAndSlash(state *State, candidates CandidateList, statistics *ValidatorStatisticList, delegations *DelegationList, distributions *DistributionList, v2_1 bool) (map[string]diadem.BigUInt, map[string]*diadem.BigUInt) {
	formerValidatorTotals := make(map[string]diadem.BigUInt)
	delegatorRewards := make(map[string]*diadem.BigUInt)
	for _, validator := range state.Validators {
		// get candidate record to lookup fee
		candidate := candidates.GetByPubKey(validator.PubKey)

		if candidate != nil {
			candidateAddress := diadem.UnmarshalAddressPB(candidate.Address)
			...
			if statistic == nil {
				...
				if common.IsZero(statistic.SlashPercentage.Value) {
					...
					validatorShare := CalculateFraction(diadem.BigUInt{big.NewInt(int64(candidate.Fee))}, statistic.DistributionTotal.Value, v2_1)
				...
				statistic.DistributionTotal = diadem.BigZeroPB()
...
func rewardValidator(statistic *ValidatorStatistic, params *Params, totalValidatorDelegations diadem.BigUInt, v2_1 bool) {
...
func slashValidatorDelegations(delegations *DelegationList, statistic *ValidatorStatistic, validatorAddress diadem.Address, v2_1 bool) {
	...
	// reset slash total
	statistic.SlashPercentage = diadem.BigZeroPB()
}
...
func distributeDelegatorRewards(ctx contract.Context, state State, formerValidatorTotals map[string]diadem.BigUInt, delegatorRewards map[string]*diadem.BigUInt, delegations *DelegationList, distributions *DistributionList, statistics *ValidatorStatisticList) (map[string]*diadem.BigUInt, error) {
	newDelegationTotals := make(map[string]*diadem.BigUInt)
	v2_1 := ctx.FeatureEnabled(diademchain.DPOSVersion2_1, false)
	...
	for _, statistic := range *statistics {
		if statistic.WhitelistAmount != nil && !common.IsZero(statistic.WhitelistAmount.Value) {
			amount := statistic.WhitelistAmount.Value
			if ctx.FeatureEnabled(diademchain.DPOSVersion2_1, false) {
				amount = calculateWeightedWhitelistAmount(*statistic)
			}
			validatorKey := diadem.UnmarshalAddressPB(statistic.Address).String()
			newDelegationTotals[validatorKey] = &amount
		}
	}

	for _, delegation := range *delegations {
		validatorKey := diadem.UnmarshalAddressPB(delegation.Validator).String()
		...
		if delegation.State == BONDING {
			...
			delegation.Amount = &types.BigUInt{Value: *updatedAmount}
			coin := loadCoin(ctx, state.Params)
			err := coin.Transfer(diadem.UnmarshalAddressPB(delegation.Delegator), &delegation.UpdateAmount.Value)
			if err != nil {
				return nil, err
			}
		...
		delegation.UpdateAmount = diadem.BigZeroPB()
...
func (c *DPOS) ClaimDistribution(ctx contract.Context, req *ClaimDistributionRequest) (*ClaimDistributionResponse, error) {
	...
	err = coin.Transfer(diadem.UnmarshalAddressPB(req.WithdrawalAddress), &distribution.Amount.Value)
	if err != nil {
		return nil, err
	}
...
func emitSlashEvent(ctx contract.Context, validator *types.Address, slashPercentage diadem.BigUInt) error {
...
func Dump(ctx contract.Context, dposv3Address diadem.Address) (*dposv3.InitializationState, error) {
	if ctx.FeatureEnabled(diademchain.DPOSVersion3Feature, false) {
