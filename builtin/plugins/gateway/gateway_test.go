...
var (
	...
	addr4 = diadem.MustParseAddress("chain:0x5cecd1f7261e1f4c684e297be3edf03b825e01c9")

	dappAccAddr1 = diadem.MustParseAddress("chain:0x5cecd1f7261e1f4c684e297be3edf03b825e01c4")
	ethAccAddr1  = diadem.MustParseAddress("eth:0x5cecd1f7261e1f4c684e297be3edf03b825e01c4")

	ethTokenAddr  = diadem.MustParseAddress("eth:0xb16a379ec18d4093666f8f38b11a3071c920207d")
	ethTokenAddr2 = diadem.MustParseAddress("eth:0xfa4c7920accfd66b86f5fd0e69682a79f762d49e")
	ethTokenAddr3 = diadem.MustParseAddress("eth:0x5d1ddf5223a412d24901c32d14ef56cb706c0f64")
	ethTokenAddr4 = diadem.MustParseAddress("eth:0xb16a379ec18d4093666f8f38b11a3071c920207d")
)
...
type GatewayTestSuite struct {
	...
	ethAddr   diadem.Address
	ethAddr2  diadem.Address
	ethAddr3  diadem.Address
	ethAddr4  diadem.Address
	dAppAddr  diadem.Address
	dAppAddr2 diadem.Address
	dAppAddr3 diadem.Address
	dAppAddr4 diadem.Address
}
...
func (ts *GatewayTestSuite) SetupTest() {
	...
	require.NoError(err)
	ethLocalAddr, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(ts.ethKey.PublicKey).Hex())
	require.NoError(err)
	ts.ethAddr = diadem.Address{ChainID: "eth", Local: ethLocalAddr}
	ts.ethKey2, err = crypto.GenerateKey()
	require.NoError(err)
	ethLocalAddr, err = diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(ts.ethKey2.PublicKey).Hex())
	require.NoError(err)
	ts.ethAddr2 = diadem.Address{ChainID: "eth", Local: ethLocalAddr}
	ts.dAppAddr = diadem.Address{ChainID: "chain", Local: addr1.Local}
	ts.dAppAddr2 = diadem.Address{ChainID: "chain", Local: addr2.Local}
	ts.dAppAddr3 = diadem.Address{ChainID: "chain", Local: addr3.Local}
	var err1 error
	ts.ethKey3, err1 = crypto.GenerateKey()
	require.NoError(err1)
	ethLocalAddr3, err1 := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(ts.ethKey3.PublicKey).Hex())
	require.NoError(err1)
	ts.ethAddr3 = diadem.Address{ChainID: "eth", Local: ethLocalAddr3}
	ts.ethKey4, err1 = crypto.GenerateKey()
	require.NoError(err1)
	ethLocalAddr4, err1 := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(ts.ethKey4.PublicKey).Hex())
	require.NoError(err1)
	ts.ethAddr4 = diadem.Address{ChainID: "eth", Local: ethLocalAddr4}
}
...
func (ts *GatewayTestSuite) TestOwnerPermissions() {
	require := ts.Require()
	fakeCtx := lp.CreateFakeContext(ts.dAppAddr, diadem.RootAddress("chain"))
...
func (ts *GatewayTestSuite) TestOutOfOrderEventBatchProcessing() {
	require := ts.Require()
	fakeCtx := plugin.CreateFakeContextWithEVM(ts.dAppAddr /*caller*/, diadem.RootAddress("chain") /*contract*/)
	...
	err = gw.ProcessEventBatch(gwCtx, &ProcessEventBatchRequest{
		FtDeposits: []*TokenDeposit{
			&TokenDeposit{
				...
				Amount:   &types.BigUInt{Value: *diadem.NewBigUIntFromInt(depositAmount)},
				EthBlock: 5,
			},
		},
	})
...
func (ts *GatewayTestSuite) TestGatewayERC721Deposit() {
	require := ts.Require()
	fakeCtx := plugin.CreateFakeContextWithEVM(ts.dAppAddr, diadem.RootAddress("chain"))
			...
			&MainnetEvent{
				...
				Payload: &MainnetDepositEvent{
					Deposit: &MainnetTokenDeposited{
						...
						TokenID:       &types.BigUInt{Value: *diadem.NewBigUIntFromInt(123)},
					},
				},
			},
		},
...
func (ts *GatewayTestSuite) TestWithdrawalRestrictions() {
	...
	err = gwHelper.Contract.WithdrawToken(
		...
		&WithdrawTokenRequest{
			TokenContract: ethTokenAddr.MarshalPB(),
			TokenKind:     TokenKind_ERC721,
			TokenID:       &types.BigUInt{Value: *diadem.NewBigUInt(token1)},
	...
	err = gwHelper.Contract.WithdrawToken(
		...
		&WithdrawTokenRequest{
			TokenContract: ethTokenAddr.MarshalPB(),
			TokenKind:     TokenKind_ERC721,
			TokenID:       &types.BigUInt{Value: *diadem.NewBigUInt(token2)},
		},
	)
	...
	err = gwHelper.Contract.WithdrawETH(
		gwHelper.ContractCtx(fakeCtx.WithSender(ts.dAppAddr)),
		&WithdrawETHRequest{
			Amount:         &types.BigUInt{Value: *diadem.NewBigUInt(ethAmt)},
	...
	err = gwHelper.Contract.WithdrawToken(
		...
		&WithdrawTokenRequest{
			TokenContract: ethTokenAddr.MarshalPB(),
			TokenKind:     TokenKind_ERC721,
			TokenID:       &types.BigUInt{Value: *diadem.NewBigUInt(token3)},
			Recipient:     ts.ethAddr2.MarshalPB(),
		},
	)
	require.Equal(ErrPendingWithdrawalExists, err)
...
func (ts *GatewayTestSuite) TestReclaimTokensAfterIdentityMapping() {
	...
	require.Equal(diadem.UnmarshalAddressPB(tokens[0].TokenContract), ethTokenAddr)
...
func (ts *GatewayTestSuite) TestGetUnclaimedContractTokens() {
	...
	DiademCoinGwHelper, err := deployGatewayContract(fakeCtx, &InitRequest{
		Owner:   ts.dAppAddr2.MarshalPB(),
		Oracles: []*types.Address{ts.dAppAddr.MarshalPB()},
	}, true)
	require.NoError(err)
	...
	erc721xAddr, err := deployTokenContract(fakeCtx, "SampleERC721XToken", gwHelper.Address, ts.dAppAddr)
	require.NoError(err)
	diademAddr, err := deployDiademCoinContract(fakeCtx)
	require.NoError(err)
	// Don't add the contract mapping between the Mainnet & DAppChain contracts...
	...
	diademamountsByBlock := []int64{160, 239, 581}
	diademdeposits := genDiademCoinDeposits(
		diademAddr.Address,
		carolEthAddr,
		[]uint64{71, 73, 75},
		diademamountsByBlock,
	)

	diademamountsByBlock2 := []int64{390}
	diademdeposits2 := genDiademCoinDeposits(
		diademAddr.Address,
		charlieEthAddr,
		[]uint64{79},
		diademamountsByBlock2,
	)

	// Send tokens to Gateway Go contract...
	// None of the tokens will be transferred to their owners because the contract mapping
	// doesn't exist.
	// Diadem Tokens will not be transferred to their owners as identity mapping does not exist, contract mapping does not apply here
	require.NoError(gwHelper.Contract.ProcessEventBatch(
		gwHelper.ContractCtx(fakeCtx),
		&ProcessEventBatchRequest{Events: erc721deposits}),
	)
	...
	require.NoError(DiademCoinGwHelper.Contract.ProcessEventBatch(
		DiademCoinGwHelper.ContractCtx(fakeCtx),
		&ProcessEventBatchRequest{Events: diademdeposits}),
	)
	require.NoError(DiademCoinGwHelper.Contract.ProcessEventBatch(
		DiademCoinGwHelper.ContractCtx(fakeCtx),
		&ProcessEventBatchRequest{Events: diademdeposits2}),
	)
	...
	diademcoin := newcoinStaticContext(DiademCoinGwHelper.ContractCtx(fakeCtx))
	bal, err = diademcoin.balanceOf(carolDAppAddr)
	require.NoError(err)
	require.Equal(int64(0), bal.Int64())
	bal, err = diademcoin.balanceOf(charlieDAppAddr)
	require.NoError(err)
	require.Equal(int64(0), bal.Int64())
	...
	require.NoError(err)
	require.Equal(diadem.NewBigUIntFromInt(13), &resp.UnclaimedAmount.Value)
	...
	require.NoError(err)
	require.Equal(diadem.NewBigUIntFromInt(1357), &resp.UnclaimedAmount.Value)
	...
	require.NoError(err)
	require.Equal(diadem.NewBigUIntFromInt(374), &resp.UnclaimedAmount.Value)
	resp, err = DiademCoinGwHelper.Contract.GetUnclaimedContractTokens(DiademCoinGwHelper.ContractCtx(fakeCtx), &GetUnclaimedContractTokensRequest{TokenAddress: diademAddr.Address.MarshalPB()})
	require.NoError(err)
	require.Equal(diadem.NewBigUIntFromInt(1370), &resp.UnclaimedAmount.Value)
}
...
func (ts *GatewayTestSuite) TestGetOracles() {
	...
	require.Equal(oracleAddr, diadem.UnmarshalAddressPB(resp.Oracles[0].Address))
	...
	addr1 := diadem.UnmarshalAddressPB(resp.Oracles[0].Address)
	addr2 := diadem.UnmarshalAddressPB(resp.Oracles[1].Address)
	...
	require.NoError(err)
	require.Len(resp.Oracles, 1)
	require.Equal(oracle2Addr, diadem.UnmarshalAddressPB(resp.Oracles[0].Address))
}
...
func (ts *GatewayTestSuite) TestAddRemoveTokenWithdrawer() {
	...
	fakeCtx := plugin.CreateFakeContextWithEVM(ownerAddr, diadem.RootAddress("chain"))
	...
	require.Equal(diadem.UnmarshalAddressPB(s.TokenWithdrawers[0]), withdrawerAddr)
...
func (ts *GatewayTestSuite) TestAddNewContractMapping() {
	...
	fakeCtx := plugin.CreateFakeContextWithEVM(userAddr, diadem.RootAddress("chain"))
...
func (ts *GatewayTestSuite) TestUnclaimedTokenMarshalling() {
	...
	original := UnclaimedToken{
		...
		Amounts: []*TokenAmount{
			&TokenAmount{
				TokenID:     &types.BigUInt{Value: *diadem.NewBigUIntFromInt(0)},
				TokenAmount: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1)},
			},
			&TokenAmount{
				TokenID:     &types.BigUInt{Value: *diadem.NewBigUIntFromInt(0)},
				TokenAmount: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(0)},
			},
		},
	}
...
func (ts *GatewayTestSuite) TestDiademCoinTG() {
	require := ts.Require()
	...
	require.EqualError(diademCoinGwHelper.Contract.WithdrawETH(diademCoinGwHelper.ContractCtx(fakeCtx.WithSender(userAddr)), &WithdrawETHRequest{
		Amount:         &types.BigUInt{Value: *diadem.NewBigUIntFromInt(0)},
		MainnetGateway: foreignCreatorAddr.MarshalPB(),
	}), ErrInvalidRequest.Error(), "WithdrawEth shouldnt happen in diademcoin TG contract")

	require.EqualError(diademCoinGwHelper.Contract.ProcessEventBatch(diademCoinGwHelper.ContractCtx(fakeCtx.WithSender(oracleAddr)), &ProcessEventBatchRequest{
		Events: genERC721Deposits(ethTokenAddr, ts.ethAddr, []uint64{9, 10}, nil),
	}), ErrInvalidRequest.Error(), "ProcessEventBatch wont entertain events other than diademcoin in diademcoin TG contract")

	require.Nil(diademCoinGwHelper.Contract.ProcessEventBatch(diademCoinGwHelper.ContractCtx(fakeCtx.WithSender(oracleAddr)), &ProcessEventBatchRequest{
		Events: genDiademCoinDeposits(ethTokenAddr, ts.ethAddr, []uint64{9, 10}, []int64{10, 11}),
	}), "ProcessEventBatch should entertain events of diademcoin in diademcoin TG")
	...
	require.Nil(gwHelper.Contract.ProcessEventBatch(gwHelper.ContractCtx(fakeCtx.WithSender(oracleAddr)), &ProcessEventBatchRequest{
		Events: genERC721Deposits(ethTokenAddr, ts.ethAddr, []uint64{9, 10}, nil),
	}), "ProcessEventBatch should entertain events other than diademcoin in TG")
	...
	require.EqualError(gwHelper.Contract.ProcessEventBatch(gwHelper.ContractCtx(fakeCtx.WithSender(oracleAddr)), &ProcessEventBatchRequest{
		Events: genDiademCoinDeposits(ethTokenAddr, ts.ethAddr, []uint64{10, 11}, []int64{10, 11}),
	}), ErrInvalidRequest.Error(), "ProcessEventBatch wont entertain events of diademcoin in TG comtract")
...
func (ts *GatewayTestSuite) TestCheckSeenTxHash() {
	...
	fakeCtx = fakeCtx.WithFeature(diademchain.TGCheckTxHashFeature, true)
	require.True(fakeCtx.FeatureEnabled(diademchain.TGCheckTxHashFeature, false))
			...
			&MainnetEvent{
				...
				Payload: &MainnetDepositEvent{
					Deposit: &MainnetTokenDeposited{
						...
						TokenContract: ethTokenAddr.MarshalPB(),
						TokenOwner:    ts.ethAddr.MarshalPB(),
						TokenID:       &types.BigUInt{Value: *diadem.NewBigUIntFromInt(100)},
						TxHash:        txHash1,
					},
				},
			},
			...
			&MainnetEvent{
				...
				Payload: &MainnetDepositEvent{
					Deposit: &MainnetTokenDeposited{
						...
						TokenContract: ethTokenAddr.MarshalPB(),
						TokenOwner:    ts.ethAddr.MarshalPB(),
						TokenID:       &types.BigUInt{Value: *diadem.NewBigUIntFromInt(200)},
						TxHash:        txHash2,
					},
				},
			},
