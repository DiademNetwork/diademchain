...
type (
	...
	WithdrawDiademCoinError              = tgtypes.TransferGatewayWithdrawDiademCoinError
	MainnetEventTxHashInfo             = tgtypes.TransferGatewayMainnetEventTxHashInfo

	WithdrawDiademCoinRequest = tgtypes.TransferGatewayWithdrawDiademCoinRequest
)
...
const (
	...
	withdrawDiademCoinTopic              = "event:WithdrawDiademCoin"
	...
	withdrawDiademCoinErrorTopic         = "event:WithdrawDiademCoinError"
...
func localAccountKey(owner diadem.Address) []byte {
	return util.PrefixKey(localAccountKeyPrefix, owner.Bytes())
}

func foreignAccountKey(owner diadem.Address) []byte {
	return util.PrefixKey(foreignAccountKeyPrefix, owner.Bytes())
}

func oracleStateKey(oracle diadem.Address) []byte {
...
func contractAddrMappingKey(contractAddr diadem.Address) []byte {
	return util.PrefixKey(contractAddrMappingKeyPrefix, contractAddr.Bytes())
}

func unclaimedTokenDepositorKey(contractAddr, ownerAddr diadem.Address) []byte {
...
func unclaimedTokenDepositorsRangePrefix(contractAddr diadem.Address) []byte {
	return util.PrefixKey(unclaimedTokenDepositorByContractPrefix, contractAddr.Bytes())
}

func unclaimedTokenKey(ownerAddr, contractAddr diadem.Address) []byte {
...
func unclaimedTokensRangePrefix(ownerAddr diadem.Address) []byte {
...
func (gw *Gateway) Init(ctx contract.Context, req *InitRequest) error {
	...
	for _, oracleAddrPB := range req.Oracles {
		oracleAddr := diadem.UnmarshalAddressPB(oracleAddrPB)
...
func (gw *Gateway) ReplaceOwner(ctx contract.Context, req *AddOracleRequest) error {
	...
	if diadem.UnmarshalAddressPB(state.Owner).Compare(ctx.Message().Sender) != 0 {
		return ErrNotAuthorized
	}
...
func removeOracle(ctx contract.Context, oracleAddr diadem.Address) error {
...
func (gw *Gateway) ProcessEventBatch(ctx contract.Context, req *ProcessEventBatchRequest) error {
	...
	checkTxHash := ctx.FeatureEnabled(diademchain.TGCheckTxHashFeature, false)
	...
	for _, ev := range req.Events {
		...
		case *tgtypes.TransferGatewayMainnetEvent_Deposit:
			...
			// If diademCoinTG flag is true, then token kind must need to be diademcoin
			// If diademCoinTG flag is false, then token kind must not be diademcoin
			if gw.diademCoinTG != (payload.Deposit.TokenKind == TokenKind_DiademCoin) {
				return ErrInvalidRequest
			}
			...
			ownerAddr := diadem.UnmarshalAddressPB(payload.Deposit.TokenOwner)
			tokenAddr := diadem.RootAddress("eth")
			if payload.Deposit.TokenContract != nil {
				tokenAddr = diadem.UnmarshalAddressPB(payload.Deposit.TokenContract)
			}
		...
		case *tgtypes.TransferGatewayMainnetEvent_Withdrawal:
			...
			// If diademCoinTG flag is true, then token kind must need to be diademcoin
			// If diademCoinTG flag is false, then token kind must not be diademcoin
			if gw.diademCoinTG != (payload.Withdrawal.TokenKind == TokenKind_DiademCoin) {
				return ErrInvalidRequest
			}
...
func (gw *Gateway) WithdrawToken(ctx contract.Context, req *WithdrawTokenRequest) error {
	...
	ownerEthAddr := diadem.Address{}
	if req.Recipient != nil {
		ownerEthAddr = diadem.UnmarshalAddressPB(req.Recipient)
	} else {
	...
	tokenAddr := diadem.UnmarshalAddressPB(req.TokenContract)
...
func (gw *Gateway) WithdrawETH(ctx contract.Context, req *WithdrawETHRequest) error {
	...
	// If diademCoinTG flag is true, then we cant allow eth withdraw operation
	if gw.diademCoinTG {
		return ErrInvalidRequest
	}
...
// WithdrawDiademCoin will attempt to transfer Diademcoin to the Gateway contract,
// if it's successful it will store a receipt than can be used by the depositor to reclaim ownership
// of the Diademcoin through the Mainnet Gateway contract.
// NOTE: Currently an entity must complete each withdrawal by reclaiming ownership on Mainnet
//       before it can make another withdrawal (even if the tokens/ETH/Diadem originate from different
//       ERC20 or ERC721 contracts).
func (gw *Gateway) WithdrawDiademCoin(ctx contract.Context, req *WithdrawDiademCoinRequest) error {
	if req.Amount == nil || req.TokenContract == nil {
		return ErrInvalidRequest
	}

	// If diademCoinTG flag is false, then we cant allow diademcoin withdraw operation
	if !gw.diademCoinTG {
		return ErrInvalidRequest
	}
	...
	account, err := loadLocalAccount(ctx, ownerAddr)
	if err != nil {
		emitWithdrawDiademCoinError(ctx, err.Error(), req)
		return err
	}

	if account.WithdrawalReceipt != nil {
		emitWithdrawDiademCoinError(ctx, ErrPendingWithdrawalExists.Error(), req)
		return ErrPendingWithdrawalExists
	}
	...
	ctx.Logger().Info("WithdrawDiademCoin", "owner", ownerEthAddr, "token", req.TokenContract)
	...
	ctx.EmitTopics(event, withdrawDiademCoinTopic)
...
func (gw *Gateway) ConfirmWithdrawalReceiptV2(ctx contract.Context, req *ConfirmWithdrawalReceiptRequest) error {
	...
	for _, v := range validators {
		if sender.Compare(diadem.UnmarshalAddressPB(v.Address)) == 0 {
			found = true
			break
		}
	}
...
func (gw *Gateway) doConfirmWithdrawalReceipt(ctx contract.Context, req *ConfirmWithdrawalReceiptRequest) error {
	...
	ownerAddr := diadem.UnmarshalAddressPB(req.TokenOwner)
	account, err := loadLocalAccount(ctx, ownerAddr)
	if err != nil {
		return err
	}
...
func (gw *Gateway) PendingWithdrawals(ctx contract.StaticContext, req *PendingWithdrawalsRequest) (*PendingWithdrawalsResponse, error) {
	...
	for _, ownerAddrPB := range state.TokenWithdrawers {
		ownerAddr := diadem.UnmarshalAddressPB(ownerAddrPB)
		account, err := loadLocalAccount(ctx, ownerAddr)
		if err != nil {
			return nil, err
		}
...
func (gw *Gateway) ReclaimDepositorTokens(ctx contract.Context, req *ReclaimDepositorTokensRequest) error {
	...
	for _, depAddr := range req.Depositors {
		ownerAddr := diadem.UnmarshalAddressPB(depAddr)
		if err := reclaimDepositorTokens(ctx, ownerAddr); err != nil {
			emitReclaimError(ctx, "[ReclaimDepositorTokens reclaimDepositorTokens] "+err.Error(), ownerAddr)
...
func (gw *Gateway) GetUnclaimedContractTokens(
	...
	ethTokenAddress := diadem.UnmarshalAddressPB(req.TokenAddress)
	depositors, err := unclaimedTokenDepositorsByContract(ctx, ethTokenAddress)
	if err != nil {
		return nil, err
	}
	unclaimedAmount := diadem.NewBigUIntFromInt(0)
	var unclaimedToken UnclaimedToken
	amount := diadem.NewBigUIntFromInt(0)
	for _, address := range depositors {
		...
		case TokenKind_ERC721:
			unclaimedAmount = unclaimedAmount.Add(unclaimedAmount, diadem.NewBigUIntFromInt(int64(len(unclaimedToken.Amounts))))
		case TokenKind_ERC721X:
			for _, a := range unclaimedToken.Amounts {
				unclaimedAmount = unclaimedAmount.Add(unclaimedAmount, diadem.NewBigUInt(a.TokenAmount.Value.Int))

			}
		case TokenKind_ERC20, TokenKind_ETH, TokenKind_DiademCoin:
			if len(unclaimedToken.Amounts) == 1 {
				amount = diadem.NewBigUInt(unclaimedToken.Amounts[0].TokenAmount.Value.Int)
			}
...
func (gw *Gateway) ReclaimContractTokens(ctx contract.Context, req *ReclaimContractTokensRequest) error {
	...
	foreignContractAddr := diadem.UnmarshalAddressPB(req.TokenContract)
	...
	if cr.CreatorAddress.Compare(callerAddr) != 0 {
		...
		if diadem.UnmarshalAddressPB(state.Owner).Compare(callerAddr) != 0 {
	...
	for _, entry := range ctx.Range(unclaimedTokenDepositorsRangePrefix(foreignContractAddr)) {
		...
		ownerAddr := diadem.UnmarshalAddressPB(&addr)
...
func reclaimDepositorTokens(ctx contract.Context, ownerAddr diadem.Address) error {
	for _, entry := range ctx.Range(unclaimedTokensRangePrefix(ownerAddr)) {
		...
		if unclaimedToken.TokenContract != nil {
			tokenAddr = diadem.UnmarshalAddressPB(unclaimedToken.TokenContract)
		}
...
func reclaimDepositorTokensForContract(
	ctx contract.Context, ownerAddr, tokenAddr diadem.Address, unclaimedToken *UnclaimedToken,
) error {
	failed := []*TokenAmount{}
	for _, a := range unclaimedToken.Amounts {
...
func transferTokenDeposit(
	ctx contract.Context, ownerEthAddr, tokenEthAddr diadem.Address,
	...
	var tokenAddr diadem.Address
	if kind != TokenKind_ETH && kind != TokenKind_DiademCoin {
	...
	case TokenKind_DiademCoin:
		...
		if availableFunds.Cmp(safeAmount) < 0 {
			if err := coin.mintToGateway(safeAmount); err != nil {
				return errors.Wrapf(err, "failed to mint diadem coin - %s", safeAmount.String())
			}
		}

		if err := coin.transfer(ownerAddr, safeAmount); err != nil {
			return errors.Wrap(err, "failed to transfer diadem coin")
		}
	}
...
func storeUnclaimedToken(ctx contract.Context, deposit *MainnetTokenDeposited) error {
	ownerAddr := diadem.UnmarshalAddressPB(deposit.TokenOwner)
	tokenAddr := diadem.RootAddress("eth")

	if deposit.TokenContract != nil {
		tokenAddr = diadem.UnmarshalAddressPB(deposit.TokenContract)
	}
	...
	case TokenKind_ERC20, TokenKind_ETH, TokenKind_DiademCoin:
		...
		unclaimedToken.Amounts = []*TokenAmount{
			&TokenAmount{
				TokenAmount: &types.BigUInt{Value: *diadem.NewBigUInt(newAmount)},
			},
		}
	}
...
func completeTokenWithdraw(ctx contract.Context, state *GatewayState, withdrawal *MainnetTokenWithdrawn) error {
	...
	ownerEthAddr := diadem.UnmarshalAddressPB(withdrawal.TokenOwner)
	foreignAccount, err := loadForeignAccount(ctx, ownerEthAddr)
	if err != nil {
		return err
	}
	...
	ownerAddr := diadem.UnmarshalAddressPB(foreignAccount.CurrentWithdrawer)
	account, err := loadLocalAccount(ctx, ownerAddr)
	if err != nil {
		return err
	}
...
func resolveToDAppAddr(ctx contract.StaticContext, mapperAddr, ethAddr diadem.Address) (diadem.Address, error) {
...
func resolveToEthAddr(ctx contract.StaticContext, mapperAddr, dappAddr diadem.Address) (diadem.Address, error) {
	var resp address_mapper.GetMappingResponse
...
func loadLocalAccount(ctx contract.StaticContext, owner diadem.Address) (*LocalAccount, error) {
...
func saveLocalAccount(ctx contract.Context, acct *LocalAccount) error {
	ownerAddr := diadem.UnmarshalAddressPB(acct.Owner)
...
func loadForeignAccount(ctx contract.StaticContext, owner diadem.Address) (*ForeignAccount, error) {
...
func addTokenWithdrawer(ctx contract.StaticContext, state *GatewayState, owner diadem.Address) error {
...
func removeTokenWithdrawer(ctx contract.StaticContext, state *GatewayState, owner diadem.Address) error {
	ownerAddrPB := owner.MarshalPB()
...
func addOracle(ctx contract.Context, oracleAddr diadem.Address) error {
...
func emitReclaimError(ctx contract.Context, errorMessage string, ownerAddress diadem.Address) error {
...
func emitWithdrawDiademCoinError(ctx contract.Context, errorMessage string, request *tgtypes.TransferGatewayWithdrawDiademCoinRequest) error {
	withdrawDiademCoinError, err := proto.Marshal(&WithdrawDiademCoinError{
		WithdrawDiademCoinRequest: request,
		ErrorMessage:            errorMessage,
	})
	if err != nil {
		return err
	}
	ctx.EmitTopics(withdrawDiademCoinError, withdrawDiademCoinErrorTopic)
	return nil
}
