...
func accountKey(owner diadem.Address) []byte {
...
func transferTokenDeposit(ctx contract.Context, ownerEthAddr, tokenEthAddr diadem.Address, kind TokenKind, value *big.Int) error {
	mapperAddr, err := ctx.Resolve("addressmapper")
	if err != nil {
		return err
	}
...
func storeUnclaimedToken(ctx contract.Context, deposit *MainnetTokenDeposited) error {
	...
	case TokenKind_ERC20, TokenKind_ETH:
		...
		unclaimedToken.Values = []*types.BigUInt{
			&types.BigUInt{Value: *diadem.NewBigUInt(newAmount)},
		}
	}
...
func loadAccount(ctx contract.StaticContext, owner diadem.Address) (*Account, error) {
