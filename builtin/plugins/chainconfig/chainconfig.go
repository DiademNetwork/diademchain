...
func getCurrentValidatorsFromDPOS(ctx contract.StaticContext) ([]diadem.Address, error) {
	...
	if ctx.FeatureEnabled(diademchain.DPOSVersion3Feature, false) {
		...
		validators := make([]diadem.Address, 0, len(resp.Statistics))
...
func getCurrentValidators(ctx contract.StaticContext) ([]diadem.Address, error) {
	if !ctx.FeatureEnabled(diademchain.ChainCfgVersion1_1, false) {
	...
	validators := make([]diadem.Address, 0, len(validatorsList))
	for _, v := range validatorsList {
		if v != nil {
			address := diadem.Address{ChainID: chainID, Local: diadem.LocalAddressFromPublicKey(v.PubKey)}
...
func getFeature(ctx contract.StaticContext, name string, curValidators []diadem.Address) (*Feature, error) {
	var feature Feature
...
func enableFeature(ctx contract.Context, name string) error {
	...
	for _, v := range feature.Validators {
		if sender.Compare(diadem.UnmarshalAddressPB(v)) == 0 {
			return ErrFeatureAlreadyEnabled
		}
	}
