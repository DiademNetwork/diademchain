...
var (
	...
	billionth                     = diadem.BigUInt{big.NewInt(1000000000)}
	defaultReferrerFee            = diadem.BigUInt{big.NewInt(300)}
...
func (c *DPOS) Delegate(ctx contract.Context, req *DelegateRequest) error {
	...
	cand := GetCandidate(ctx, diadem.UnmarshalAddressPB(req.ValidatorAddress))
...
func (c *DPOS) Redelegate(ctx contract.Context, req *RedelegateRequest) error {
	...
	if req.ValidatorAddress.Local.Compare(limboValidatorAddress.Local) != 0 {
		candidate := GetCandidate(ctx, diadem.UnmarshalAddressPB(req.ValidatorAddress))
		// Delegations can only be made to existing candidates
		if candidate == nil {
			return logDposError(ctx, errCandidateNotFound, req.String())
		}
...
func (c *DPOS) addCandidateToStatisticList(ctx contract.Context, req *WhitelistCandidateRequest) error {
	_, err := GetStatistic(ctx, diadem.UnmarshalAddressPB(req.CandidateAddress))
...
func (c *DPOS) RemoveWhitelistedCandidate(ctx contract.Context, req *RemoveWhitelistedCandidateRequest) error {
	...
	statistic, err := GetStatistic(ctx, diadem.UnmarshalAddressPB(req.CandidateAddress))
...
func (c *DPOS) ListCandidates(ctx contract.StaticContext, req *ListCandidatesRequest) (*ListCandidatesResponse, error) {
	...
	for _, candidate := range candidates {
		statistic, err := GetStatistic(ctx, diadem.UnmarshalAddressPB(candidate.Address))
...
func Elect(ctx contract.Context) error {
	...
	for _, res := range delegationResults[:validatorCount] {
		...
		if candidate != nil && common.IsPositive(res.DelegationTotal) {
			...
			statistic, _ := GetStatistic(ctx, diadem.UnmarshalAddressPB(candidate.Address))
			if statistic == nil {
...
func loadCoin(ctx contract.Context) (*ERC20, error) {
	...
	coinAddr := diadem.UnmarshalAddressPB(state.Params.CoinContractAddress)
	return &ERC20{
		Context:         ctx,
	...
	for _, validator := range state.Validators {
		...
		if statistic == nil {
			...
			if common.IsZero(statistic.SlashPercentage.Value) {
				...
				validatorShare := CalculateFraction(diadem.BigUInt{big.NewInt(int64(candidate.Fee))}, distributionTotal)
				...
				for _, d := range delegations {
					if d.Validator.Local.Compare(candidate.Address.Local) == 0 {
						...
						referrerReward = CalculateFraction(diadem.BigUInt{big.NewInt(int64(candidate.Fee))}, referrerReward)
...
// the referrers
func calculateRewards(delegationTotal diadem.BigUInt, params *Params, totalValidatorDelegations diadem.BigUInt) diadem.BigUInt {
...
func slashValidatorDelegations(ctx contract.Context, statistic *ValidatorStatistic, validatorAddress diadem.Address) error {
	delegations, err := loadDelegationList(ctx)
	if err != nil {
		return err
	}
...
func distributeDelegatorRewards(ctx contract.Context, formerValidatorTotals map[string]diadem.BigUInt, delegatorRewards map[string]*diadem.BigUInt) (map[string]*diadem.BigUInt, error) {
