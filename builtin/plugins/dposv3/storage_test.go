...
var (
	...
	address1 = diadem.MustParseAddress("default:0x27690aE5F91C620B13266dA9044b8c0F35CeFbdC")
	pub2     = []byte("5wYR5atUGpnpZ+oerOZ8hi3B4dddSlxe6Hd30ZuuYWgps")
	address2 = diadem.MustParseAddress("default:0x7278ec96B05B44c643c07005577e9F060dAef4EF")
	pub3     = []byte("oTFzT+lt+ztuUQd9yuQbPAdZPmezuoOtOFCUULSqgmU=")
	address3 = diadem.MustParseAddress("default:0x8364d3A808D586b908b6B18Fc726ff134408fbfE")
	pub4     = []byte("emvRy1THBgGbNw/j1m5hqpXaVIZLHVz/GHQ58mxyc3A=")
	address4 = diadem.MustParseAddress("default:0x9c285B0CE29E29C557a06Ca3a27cf1F550a96f38")
)
...
func TestGetSetCandidateList(t *testing.T) {
	...
	// get
	cand1 := cands.Get(diadem.Address{ChainID: chainID, Local: address1.Local})
	assert.NotNil(t, cand1)
	assert.Equal(t, cand1.PubKey, pub1)
	assert.Equal(t, 0, cand1.Address.Local.Compare(address1.Local))

	cand4 := cands.Get(diadem.Address{ChainID: chainID, Local: address4.Local})
	assert.Nil(t, cand4)
...
func TestCandidateDelete(t *testing.T) {
	...
	cand2 := cands.Get(diadem.Address{ChainID: chainID, Local: address2.Local})
...
func TestGetSetStatistics(t *testing.T) {
	...
	statistic := ValidatorStatistic{
		...
		WhitelistAmount: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1)},
	}
	...
	assert.Equal(t, 0, s.WhitelistAmount.Value.Cmp(diadem.NewBigUIntFromInt(1)))

	statistic2 := ValidatorStatistic{
		Address:         address2.MarshalPB(),
		WhitelistAmount: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(10)},
	}
	...
	statistic.WhitelistAmount = &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5)}
	err = SetStatistic(ctx, &statistic)
	assert.Nil(t, err)
	...
	assert.Equal(t, 0, s.WhitelistAmount.Value.Cmp(diadem.NewBigUIntFromInt(10)))
	...
	assert.Equal(t, 0, s.Address.Local.Compare(address1.Local))
	assert.Equal(t, 0, s.WhitelistAmount.Value.Cmp(diadem.NewBigUIntFromInt(5)))
}
...
func TestAddAndSortDelegationList(t *testing.T) {
	...
	pctx := plugin.CreateFakeContext(diadem.UnmarshalAddressPB(address1), diadem.UnmarshalAddressPB(address1))
	ctx := contractpb.WrapPluginContext(pctx)

	SetDelegation(ctx, &Delegation{
		Validator: address2,
		Delegator: address2,
		Amount:    &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1)},
	})
	SetDelegation(ctx, &Delegation{
		Validator: address2,
		Delegator: address3,
		Amount:    &types.BigUInt{Value: *diadem.NewBigUIntFromInt(3)},
	})
	SetDelegation(ctx, &Delegation{
		Validator: address1,
		Delegator: address4,
		Amount:    &types.BigUInt{Value: *diadem.NewBigUIntFromInt(10)},
	})
	...
	assert.Equal(t, delegation0.Amount, &types.BigUInt{Value: *diadem.NewBigUIntFromInt(1)})

	// add updated entry
	SetDelegation(ctx, &Delegation{
		Validator: address2,
		Delegator: address2,
		Amount:    &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5)},
	})
	...
	// should contain updated value, not original value
	assert.Equal(t, delegation1.Amount, &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5)})
