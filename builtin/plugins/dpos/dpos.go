...
import (
	...
	dtypes "github.com/diademnetwork/go-diadem/builtin/types/dpos"
...
func (c *DPOS) Vote(ctx contract.Context, req *dtypes.VoteRequest) error {
	...
	candAddr := diadem.UnmarshalAddressPB(req.CandidateAddress)
	cand := cands.Get(candAddr)
	if cand == nil {
		return errCandidateNotRegistered
	}
...
func (c *DPOS) Elect(ctx contract.Context, req *ElectRequest) error {
	...
	for _, cand := range cands {
		votes, err := loadVoteSet(ctx, diadem.UnmarshalAddressPB(cand.Address))
		if err != nil {
			return err
		}
		for _, vote := range votes {
			voter, err := loadVoter(ctx, diadem.UnmarshalAddressPB(vote.VoterAddress), params.VoteAllocation)
			if err != nil {
				return err
			}
			...
			fullVotes = append(fullVotes, &FullVote{
				CandidateAddress: diadem.UnmarshalAddressPB(vote.CandidateAddress),
...
func balanceToPower(n *diadem.BigUInt) uint64 {
	// TODO: make this configurable
	div := sciNot(1, decimals)
	ret := diadem.NewBigUInt(n.Int)
	return ret.Div(ret, div).Uint64()
}

func calcVotePower(
	ctx contract.StaticContext,
	coinAddr diadem.Address,
	voter *dtypes.Voter,
) (uint64, error) {
	coin := ERC20Static{
		StaticContext:   ctx,
	...
	total, err := coin.BalanceOf(diadem.UnmarshalAddressPB(voter.Address))
	if err != nil {
		return 0, err
	}
