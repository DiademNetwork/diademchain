...
import (
	...
	"github.com/diademnetwork/go-diadem/client/plasma_cash/eth"
...
func (w *PlasmaBlockWorker) syncPlasmaBlocksWithEthereum() error {
	...
	curDiademPlasmaBlockNum, err := w.dappPlasmaClient.CurrentPlasmaBlockNum()
	if err != nil {
		return err
	}
	...
	w.status.LastSeenDAppChainPlasmaBlockNum = curDiademPlasmaBlockNum
	w.statusRwMutex.Unlock()

	if curDiademPlasmaBlockNum.Cmp(curEthPlasmaBlockNum) == 0 {
	...
	if unsubmittedPlasmaBlockNum.Cmp(curDiademPlasmaBlockNum) > 0 {
...
// TODO: This function should be moved to diademchain/builtin/plasma_cash when the Oracle is
//       integrated into diademchain.
