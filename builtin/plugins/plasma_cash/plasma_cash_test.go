...
func TestPlasmaCashSMT(t *testing.T) {
	...
	contractAddr := diadem.RootAddress("eth")
	...
	req := &PlasmaTxRequest{
		Plasmatx: &PlasmaTx{
			...
			PreviousBlock: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(0)},
			Denomination:  &types.BigUInt{Value: *diadem.NewBigUIntFromInt(0)},
		},
	}
	...
	require.Nil(t, err)
	assert.Equal(t, diadem.UnmarshalAddressPB(transferConfirmed.From).String(), generatedSender.String())
	assert.Equal(t, diadem.UnmarshalAddressPB(transferConfirmed.To).String(), addr3.String())
	...
	reqBlock := &GetBlockRequest{
		BlockHeight: &types.BigUInt{
			Value: *diadem.NewBigUIntFromInt(1000),
		},
	}
...
func TestRLPEncodings(t *testing.T) {
	address, err := diadem.LocalAddressFromHexString("0x5194b63f10691e46635b27925100cfc0a5ceca62")
	require.Nil(t, err)
	...
	plasmaTx := &PlasmaTx{
		...
		Denomination: &types.BigUInt{
			Value: *diadem.NewBigUIntFromInt(1),
		},
...
func TestPlasmaCashBalanceAfterDeposit(t *testing.T) {
	...
	tokenIDs := []*types.BigUInt{
		&types.BigUInt{Value: *diadem.NewBigUIntFromInt(721)},
		&types.BigUInt{Value: *diadem.NewBigUIntFromInt(127)},
	}
		...
		Requests: []*pctypes.PlasmaCashRequest{
			&pctypes.PlasmaCashRequest{
				Data: &pctypes.PlasmaCashRequest_Deposit{&pctypes.DepositRequest{
					Slot:         123,
					DepositBlock: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(3)},
		...
		Requests: []*pctypes.PlasmaCashRequest{
			&pctypes.PlasmaCashRequest{
				Data: &pctypes.PlasmaCashRequest_Deposit{&pctypes.DepositRequest{
					Slot:         5,
					DepositBlock: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(3)},
					Denomination: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(100)},
					From:         addr2.MarshalPB(),
					Contract:     addr3.MarshalPB(),
				}},
				Meta: &pctypes.PlasmaCashEventMeta{
	...
	err = plasmaContract.ProcessRequestBatch(ctx, &pctypes.PlasmaCashRequestBatch{
		Requests: []*pctypes.PlasmaCashRequest{
			&pctypes.PlasmaCashRequest{
				Data: &pctypes.PlasmaCashRequest_Deposit{&pctypes.DepositRequest{
					Slot:         7,
					DepositBlock: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(4)},
					Denomination: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(200)},
					From:         addr2.MarshalPB(),
					Contract:     addr3.MarshalPB(),
				}},
				Meta: &pctypes.PlasmaCashEventMeta{
	...
	err = plasmaContract.ProcessRequestBatch(ctx, &pctypes.PlasmaCashRequestBatch{
		Requests: []*pctypes.PlasmaCashRequest{
			&pctypes.PlasmaCashRequest{
				Data: &pctypes.PlasmaCashRequest_Deposit{&pctypes.DepositRequest{
					Slot:         8,
					DepositBlock: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(5)},
					Denomination: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(200)},
...
func TestGetPlasmaTxRequestOnDepositBlock(t *testing.T) {
	...
	reqPlasmaTx.BlockHeight = &types.BigUInt{
		Value: *diadem.NewBigUIntFromInt(3),
	}
...
func setupPlasmaTxAuth(ctx contractpb.Context, plasmaTx *PlasmaTx, dappchainAddr diadem.Address) (diadem.Address, error) {
	ethPrivKey, err := crypto.GenerateKey()
	if err != nil {
...
func setupPlasmaTxAuthWithKey(ctx contractpb.Context, ethPrivKey *ecdsa.PrivateKey, plasmaTx *PlasmaTx, dappchainAddr diadem.Address) (diadem.Address, error) {
	...
	ethAddress := diadem.MustParseAddress(fmt.Sprintf("eth:%s", ethLocalAddress.Hex()))
...
func registerAddressMapping(ctx contractpb.Context, from, to diadem.Address, key *ecdsa.PrivateKey) error {
...
func generateAddressMappingSignature(from, to diadem.Address, key *ecdsa.PrivateKey) ([]byte, error) {
...
func getHashAndSignature(plasmatx *PlasmaTx, privateKey *ecdsa.PrivateKey) ([]byte, []byte, error) {
	...
	calculatedPlasmaTxHash, err := diademTx.Hash()
	if err != nil {
