...
func (c *PlasmaCash) registerOracle(ctx contract.Context, pbOracle *types.Address, currentOracle *diadem.Address) error {
	if pbOracle == nil {
...
func (c *PlasmaCash) SubmitBlockToMainnet(ctx contract.Context, req *SubmitBlockToMainnetRequest) (*SubmitBlockToMainnetResponse, error) {
	...
	if len(pending.Transactions) == 0 {
		...
		pbk.CurrentHeight.Value = *diadem.NewBigUIntFromInt(roundedInt)
...
func (c *PlasmaCash) verifyPlasmaRequest(ctx contract.Context, req *PlasmaTxRequest) error {
	...
	claimedSender := diadem.UnmarshalAddressPB(req.Plasmatx.Sender)
...
func (c *PlasmaCash) PlasmaTxRequest(ctx contract.Context, req *PlasmaTxRequest) error {
	...
	sender := diadem.UnmarshalAddressPB(req.Plasmatx.Sender)
	...
	receiver := diadem.UnmarshalAddressPB(req.Plasmatx.NewOwner)
...
func (c *PlasmaCash) depositRequest(ctx contract.Context, req *DepositRequest) error {
	...
	ownerAddr := diadem.UnmarshalAddressPB(req.From)
...
func (c *PlasmaCash) GetUserSlotsRequest(ctx contract.StaticContext, req *GetUserSlotsRequest) (*GetUserSlotsResponse, error) {
	...
	reqAcct, err := loadAccount(ctx, diadem.UnmarshalAddressPB(req.From))
	if err != nil {
		return nil, err
	}
...
func transferCoin(ctx contract.Context, coin *Coin, sender, receiver diadem.Address) error {
	if coin.State != CoinState_DEPOSITED {
