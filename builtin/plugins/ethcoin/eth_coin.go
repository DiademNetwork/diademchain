...
import (
	...
	ectypes "github.com/diademnetwork/go-diadem/builtin/types/ethcoin"
...
func BalanceOf(ctx contract.StaticContext, owner diadem.Address) (*diadem.BigUInt, error) {
...
func AddBalance(ctx contract.Context, addr diadem.Address, amount *diadem.BigUInt) error {
...
func SubBalance(ctx contract.Context, addr diadem.Address, amount *diadem.BigUInt) error {
	account, err := loadAccount(ctx, addr)
	if err != nil {
		return err
	}
...
func SetBalance(ctx contract.Context, addr diadem.Address, amount *diadem.BigUInt) error {
	account, err := loadAccount(ctx, addr)
	if err != nil {
		return err
	}
	account.Balance.Value = *diadem.NewBigUInt(amount.Int)
	err = saveAccount(ctx, account)
	if err != nil {
		return err
	}
...
func Transfer(ctx contract.Context, from, to diadem.Address, amount *diadem.BigUInt) error {
...
func loadAllowance(ctx contract.StaticContext, owner, spender diadem.Address) (*Allowance, error) {
	allow := &Allowance{
		Owner:   owner.MarshalPB(),
