...
type benchmarkFunc func(state diademchain.State) error
		...
		for pctTick := pcentDeactivateTicks; pctTick >= 0; pctTick-- {
			...
			for logUsers := float64(0); logUsers < maxLogUsers; logUsers++ {
				for logContracts := 0; logContracts < maxLogContracts; logContracts++ {
					...
					karmaState := state.WithPrefix(diadem.DataPrefix(karmaAddr))
...
func addMockUsersWithContracts(b *testing.B, karmaState diademchain.State, reg registry.Registry, logUsers float64, pctUsersHaveKarma int, logContracts int) {
	...
	userHaveState := ktypes.KarmaState{
		SourceStates: []*ktypes.KarmaSource{{
			...
			Count: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(99999999)},
		}},
		DeployKarmaTotal: &types.BigUInt{Value: *diadem.NewBigUIntFromInt(99999999)},
	}
	...
	userHaveNotState := ktypes.KarmaState{
		SourceStates: []*ktypes.KarmaSource{{
			Name:  karma.CoinDeployToken,
			Count: diadem.BigZeroPB(),
		}},
		DeployKarmaTotal: diadem.BigZeroPB(),
	}
...
func userAddr(user uint64) diadem.Address {
	tail := strconv.FormatUint(user, 10) + "END"
	tail += strings.Repeat("0", 20-len(tail))
	return diadem.MustParseAddress("chain:0x" + hex.EncodeToString([]byte(tail)))
}

func MockDeployEvmContract(b *testing.B, karmaState diademchain.State, owner diadem.Address, nonce uint64, reg registry.Registry) diadem.Address {
...
func MockDeployEvmContractT(t *testing.T, karmaState diademchain.State, owner diadem.Address, nonce uint64, reg registry.Registry) diadem.Address {
	contractAddr := plugin.CreateAddress(owner, nonce)
	err := reg.Register("", contractAddr, owner)
	require.NoError(t, err)
...
func addMockUsersWithContractsT(t *testing.T, karmaState diademchain.State, reg registry.Registry, logUsers float64, pctUsersHaveKarma int, logContracts int) {
	users := uint64(math.Pow(10, float64(logUsers)))
	usersWith := uint64(float64(users) * float64(pctUsersHaveKarma) / 100)
	numContracts := uint64(math.Pow(10, float64(logContracts)))
...
func addUsers(state diademchain.State, logSize int) diademchain.State {
