...
import (
	...
	am "github.com/diademnetwork/go-diadem/client/address_mapper"
	"github.com/diademnetwork/go-diadem/client/dposv2"
	gw "github.com/diademnetwork/go-diadem/client/gateway"
	"github.com/diademnetwork/go-diadem/client/native_coin"
...
const DiademGatewayName = "diademcoin-gateway"

const getOraclesCmdExample = `
./diadem gateway get-oracles gateway --key path/to/diadem_priv.key
`

const getStateCmdExample = `
./diadem gateway get-state gateway --key path/to/diadem_priv.key
`

const addOracleCmdExample = `
./diadem gateway add-oracle <owner hex address> gateway --key path/to/diadem_priv.key
`

const removeOracleCmdExample = `
./diadem gateway remove-oracle <owner hex address> gateway --key path/to/diadem_priv.key
`

const replaceOwnerCmdExample = `
./diadem gateway replace-owner <owner hex address> gateway --key path/to/diadem_priv.key
`

const withdrawFundsCmdExample = `
./diadem gateway withdraw-funds -u http://plasma.dappchains.com:80 --chain default --key path/to/diadem_priv.key OR
./diadem gateway withdraw-funds -u http://plasma.dappchains.com:80 --chain default --hsm path/to/hsm.json
`
...
func newReplaceOwnerCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			newOwner, err := hexToDiademAddress(args[0])
			if err != nil {
			...
			if len(args) <= 1 || (strings.Compare(args[1], GatewayName) == 0) {
				name = GatewayName
			} else if strings.Compare(args[1], DiademGatewayName) == 0 {
...
func newRemoveOracleCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			oracleAddress, err := hexToDiademAddress(args[0])
			if err != nil {
				return errors.Wrap(err, "failed to add new owner")
			}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 0 || (strings.Compare(args[0], GatewayName) == 0) {
				name = GatewayName
			} else if strings.Compare(args[0], DiademGatewayName) == 0 {
				name = DiademGatewayName
			} else {
				return errors.New("Invalid gateway name")
			}
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			for i, oracle := range oracles {
				fmt.Printf("Oracle %d: %s\n", i, diadem.UnmarshalAddressPB(oracle.Address))
			}
			return err
		},
	}
...
func newWithdrawFundsToMainnetCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			 ...
			 * 5. Call withdrawDiadem transaction on the dappchain
			...
			mainnetDiademAddress := "0xa4e8c3ec456107ea67d3075bf9e3df3a75823db0"
			...
			rpcClient := getDAppChainClient()
			diademcoin, err := native_coin.ConnectToDAppChainDiademContract(rpcClient)
			if err != nil {
				return err
			}
			...
			gateway, err := gw.ConnectToDAppChainDiademGateway(rpcClient, "")
			if err != nil {
				return err
			}
			...
			ethAddr, err := addressMapper.GetMappedAccount(id.DiademAddr)
			if err != nil {
				return err
			}

			balanceBefore, err := diademcoin.BalanceOf(id)
			if err != nil {
				return err
			}
			...
			if unclaimedRewards != nil {
				resp, err := dpos.ClaimRewards(id, id.DiademAddr)
				if err != nil {
					return err
				}
				fmt.Println("Claimed rewards:", resp)

				balanceAfter, err = diademcoin.BalanceOf(id)
				if err != nil {
					return err
				}
			...
			if receipt == nil {
				...
				// Approve
				err = diademcoin.Approve(id, gatewayAddr, amount)
				if err != nil {
					return err
				}
				...
				// Get the diadem tokens to the gateway
				err = gateway.WithdrawDiadem(id, amount, common.HexToAddress(mainnetDiademAddress))
				if err != nil {
					return err
				}
			...
			tx, err := mainnetGateway.UnsignedWithdrawERC20(id, receipt.TokenAmount.Value.Int, sig, common.HexToAddress(mainnetDiademAddress))
			if err != nil {
				return err
			}
			...
			fmt.Println("Sign it with the account", ethAddr.Local.String(), "and it will authorize a DIADEM token withdrawal to you.")
