...
func AddIdentityMappingCmd() *cobra.Command {
	var chainId string
	cmd := &cobra.Command{
		Use:   "add-identity-mapping <diadem-addr> <eth-key-file>",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			ethLocalAddr, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(ethKey.PublicKey).Hex())
			if err != nil {
				return errors.Wrapf(err, "bad ethereum private key from file%v", args[1])
			}
			ethAddr := diadem.Address{ChainID: chainId, Local: ethLocalAddr}
