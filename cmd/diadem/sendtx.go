...
func deployGoTx(initFile, privFile, pubFile, algo, callerChainID string) error {
	...
	for _, contract := range gen.Contracts {
		...
		addr := diadem.UnmarshalAddressPB(response.Contract)
...
func deployTx(bcFile, privFile, pubFile, name, algo, callerChainID string) (diadem.Address, []byte, []byte, error) {
	clientAddr, signer, err := caller(privFile, pubFile, algo, callerChainID)
	if err != nil {
		return *new(diadem.Address), nil, nil, errors.Wrapf(err, "initialization failed")
	}
	if signer == nil {
		return *new(diadem.Address), nil, nil, fmt.Errorf("invalid private key")
	}

	bytetext, err := ioutil.ReadFile(bcFile)
	if err != nil {
		return *new(diadem.Address), nil, nil, errors.Wrapf(err, "reading deployment file")
	}
	...
	if err != nil {
		return *new(diadem.Address), nil, nil, errors.Wrapf(err, "decoding the data in deployment file")
	}

	rpcclient := client.NewDAppChainRPCClient(cli.TxFlags.ChainID, cli.TxFlags.URI+"/rpc", cli.TxFlags.URI+"/query")
	respB, err := rpcclient.CommitDeployTx(clientAddr, signer, vm.VMType_EVM, bytecode, name)
	if err != nil {
		return *new(diadem.Address), nil, nil, errors.Wrapf(err, "CommitDeployTx")
	}
	response := vm.DeployResponse{}
	err = proto.Unmarshal(respB, &response)
	if err != nil {
		return *new(diadem.Address), nil, nil, errors.Wrapf(err, "unmarshalling response")
	}
	addr := diadem.UnmarshalAddressPB(response.Contract)
...
func staticCallTx(addr, name, input, privFile, publicFile, algo, callerChainID string) ([]byte, error) {
	rpcclient := client.NewDAppChainRPCClient(cli.TxFlags.ChainID, cli.TxFlags.URI+"/rpc", cli.TxFlags.URI+"/query")
	var contractLocalAddr diadem.LocalAddress
	var err error
	if addr != "" {
		contractLocalAddr, err = diadem.LocalAddressFromHexString(addr)
		if name != "" {
...
func callTx(addr, name, input, privFile, publicFile, algo, callerChainID string) ([]byte, error) {
	rpcclient := client.NewDAppChainRPCClient(cli.TxFlags.ChainID, cli.TxFlags.URI+"/rpc", cli.TxFlags.URI+"/query")
	var contractAddr diadem.Address
	var err error
	if addr != "" {
		if name != "" {
			fmt.Println("Both name and address entered, using address ", addr)
...
func caller(privKeyB64, publicKeyB64, algo, chainID string) (diadem.Address, auth.Signer, error) {
	var err error
	localAddr := []byte{}
	if len(publicKeyB64) > 0 {
		...
		if err == nil {
			...
			if err == nil {
				localAddr = diadem.LocalAddressFromPublicKey(addr)
			}
		}
	}
	...
	var clientAddr diadem.Address
	if len(localAddr) == 0 {
		clientAddr = diadem.RootAddress(chainID)
	} else {
		clientAddr = diadem.Address{
			ChainID: chainID,
			Local:   localAddr,
		}
	}
...
func ed25519Signer(keyFilename string) ([]byte, auth.Signer, error) {
	...
	localAddr := diadem.LocalAddressFromPublicKey(signer.PublicKey())
...
func secp256k1Signer(keyFilename string) ([]byte, auth.Signer, error) {
	...
	localAddr, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(key.PublicKey).Hex())
	if err != nil {
