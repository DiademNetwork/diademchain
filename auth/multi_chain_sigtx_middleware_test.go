...
var (
	...
	addr1    = diadem.MustParseAddress(defaultDiademChainId + ":" + pubKey1)
	origin   = diadem.MustParseAddress(defaultDiademChainId + ":0x5cecd1f7261e1f4c684e297be3edf03b825e01c4")
	contract = diadem.MustParseAddress(defaultDiademChainId + ":0x9a1aC42a17AAD6Dbc6d21c162989d0f701074044")
)
...
func TestSigning(t *testing.T) {
	...
	ethLocalAdr, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(privateKey.PublicKey).Hex())
	require.NoError(t, err)
...
func TestTronSigning(t *testing.T) {
	...
	ethLocalAdr2, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(*UnmarshalPubkey).Hex())
	require.NoError(t, err)
...
func TestEthAddressMappingVerification(t *testing.T) {
	...
	// Normal diadem transaction without address mapping
	...
	require.NoError(t, err)
	ethLocalAdr, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(ethKey.PublicKey).Hex())
	require.NoError(t, err)
	ethPublicAddr := diadem.Address{ChainID: "eth", Local: ethLocalAdr}
	...
	// set up address mapping between eth and diadem accounts
	...
	// tx using address mapping from eth account. No error this time as mapped diadem account is found.
	txSigned = mockSignedTx(t, "eth", &auth.EthSigner66Byte{ethKey})
	_, err = throttleMiddlewareHandler(tmx, state, txSigned, ctx)
	require.NoError(t, err)
}
...
func throttleMiddlewareHandler(ttm diademchain.TxMiddlewareFunc, state diademchain.State, signedTx []byte, ctx context.Context) (diademchain.TxHandlerResult, error) {
	return ttm.ProcessTx(state.WithContext(ctx), signedTx,
		func(state diademchain.State, txBytes []byte, isCheckTx bool) (res diademchain.TxHandlerResult, err error) {
			...
			return diademchain.TxHandlerResult{}, err
		}, false,
	)
}
...
func mockSignedTx(t *testing.T, chainID string, signer auth.Signer) []byte {
	...
	require.NoError(t, err)
	ethLocalAdr, err := diadem.LocalAddressFromHexString(crypto.PubkeyToAddress(*privateKey).Hex())
	require.NoError(t, err)
	nonceTx := mockNonceTx(t, diadem.Address{ChainID: chainID, Local: ethLocalAdr}, sequence)
...
func mockNonceTx(t *testing.T, from diadem.Address, sequence uint64) []byte {
	...
	require.NoError(t, err)
	tx, err := proto.Marshal(&diademchain.Transaction{
		Id:   callId,
		Data: messageTx,
	})
	require.Nil(t, err)
