...
const (
	DiademSignedTxType     SignedTxType = "diadem"
...
var originRecoveryFuncs = map[SignedTxType]originRecoveryFunc{
	DiademSignedTxType:     verifyEd25519,
...
func NewMultiChainSignatureTxMiddleware(
	...
	return diademchain.TxMiddlewareFunc(func(
		...
		msgSender := diadem.UnmarshalAddressPB(msg.From)
...
func getMappedAccountAddress(
	...
	if err != nil {
		return diadem.Address{}, errors.Wrap(err, "failed to create Address Mapper context")
	}
	...
	if err != nil {
		return diadem.Address{}, errors.Wrapf(err, "failed to map account %s", addr.String())
	}

	mappedAddr := diadem.UnmarshalAddressPB(resp.To)
	if mappedAddr.ChainID != state.Block().ChainID {
		return diadem.Address{}, fmt.Errorf("mapped account %s has wrong chain ID", addr.String())
	}
...
func verifyEd25519(tx SignedTx) ([]byte, error) {
	...
	return diadem.LocalAddressFromPublicKey(tx.PublicKey), nil
}
