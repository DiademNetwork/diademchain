...
import (
	...
	pv "github.com/diademnetwork/diademchain/privval"
...
type Backend interface {
	...
	Init() (*diadem.Validator, error)
	Reset(height uint64) error
	Destroy() error
	Start(app abci.Application) error
	RunForever()
	GenesisValidators() []*diadem.Validator
...
func (b *TendermintBackend) Init() (*diadem.Validator, error) {
	...
	genDoc := types.GenesisDoc{
		...
		GenesisTime: time.Now(), //Note this has to match on the entire cluster, TODO probably should move this to diadem.yaml
	}
...
func (b *TendermintBackend) GenesisValidators() []*diadem.Validator {
...
func (b *TendermintBackend) Start(app abci.Application) error {
	...
	validators := make([]*diadem.Validator, 0)
	for _, validator := range genDoc.Validators {
		pubKey := [ed25519.PubKeyEd25519Size]byte(validator.PubKey.(ed25519.PubKeyEd25519))
		validators = append(validators, &diadem.Validator{
			PubKey: pubKey[:],
			Power:  validator.Power,
		})
	}
