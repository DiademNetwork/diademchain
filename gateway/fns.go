...
func CreateBatchSignWithdrawalFn(isDiademcoinFn bool, chainID string, fnRegistry fnConsensus.FnRegistry, tgConfig *TransferGatewayConfig, signer auth.Signer) (*BatchSignWithdrawalFn, error) {
	...
	logger := diadem.NewDiademLogger(fnConfig.LogLevel, fnConfig.LogDestination)

	var goGateway *DAppChainGateway

	if isDiademcoinFn {
		goGateway, err = ConnectToDAppChainDiademCoinGateway(dappClient, caller, signer, logger)
		if err != nil {
			return nil, errors.Wrap(err, "failed to create dappchain diademcoin gateway")
		}
	} else {
