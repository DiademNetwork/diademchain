...
type JsonTxReceipt struct {
	...
	LogsBdiadem         Data      `json:"logsBdiadem,omitempty"`
...
func EncTxReceipt(receipt types.EvmTxReceipt) JsonTxReceipt {
	jReceipt := JsonTxReceipt{
		...
		LogsBdiadem:         EncBytes(receipt.LogsBdiadem),
...
func DecDataToAddress(chainID string, value Data) (diadem.Address, error) {
	local, err := diadem.LocalAddressFromHexString(string(value))
	if err != nil {
