...
import (
	...
	llog "github.com/diademnetwork/diademchain/log"
...
func (s *stateProvider) ReadOnlyState() diademchain.State {
	return diademchain.NewStoreState(
		nil,
		store.NewMemStore(),
		abci.Header{
			ChainID: s.ChainID,
...
func testQueryServerContractQuery(t *testing.T) {
	...
	bus := &QueryEventBus{
		Subs:    *diademchain.NewSubscriptionSet(),
...
func testQueryMetric(t *testing.T) {
	...
	wkey := `diademchain_query_service_request_count{error="false",method="Nonce"} 2`
	require.Contains(t, string(data), wkey, "want metric got none")
	wkey = `diademchain_query_service_request_count{error="true",method="Query"} 2`
	require.Contains(t, string(data), wkey, "want metric got none")
}
