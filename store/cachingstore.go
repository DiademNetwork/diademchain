...
func convertToBigCacheConfig(config *CachingStoreConfig, logger *diadem.Logger) (*bigcache.Config, error) {
...
func NewCachingStore(source VersionedKVStore, config *CachingStoreConfig) (*CachingStore, error) {
	...
	cacheLogger := diadem.NewDiademLogger(config.LogLevel, config.LogDestination)
